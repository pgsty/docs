...
| [`prometheus_port`](#prometheus_port)                       | [`PROMETHEUS`](#prometheus)       | `port`       | G     | prometheus 监听端口号，默认为 `9058`           |
...
在 3.7 版本中新加入此参数，此前的默认值为 `9090`。
不使用先前默认值 `9090` 的原因是 EL10 在此端口上启动了一个默认 Web 服务。
...
默认值自 Pigsty v3.7 以来从 `9093` 修改为 `9059`，因为 Kafka 的默认端口之一也使用 `9093`。
