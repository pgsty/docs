...
| 版本              |    发布时间    | 摘要                                                      | 地址                                                                                        |
|:----------------|:----------:|---------------------------------------------------------|-------------------------------------------------------------------------------------------|
| [v3.6.0](#v360) | 2025-07-20 | pgactive，MinIO / ETCD 改进，安装简化，配置梳理                      | [v3.6.0](https://github.com/pgsty/pigsty/releases/tag/v3.6.0)                             |
...
* `nginx_cert_validity`： 新增参数，控制 Nginx 自签名证书有效期，默认为 `397d`。
* `minio_buckets`： 的默认值调整为创建名为 `pgsql`, `meta`, `data` 的三个桶。
* `minio_users`： 移除 `dba` 用户，新增 `s3user_meta`  与 `s3user_data` 用户，分别对应 `meta` 与 `data` 桶。
* `minio_https`： 新增参数，允许配置 MinIO 使用 HTTP 模式。
* `minio_provision`： 新增参数，允许跳过 MinIO 置备阶段（跳过桶，用户创建）
...
* `etcd_learner`： 新增参数，允许 etcd 以学习者的身份初始化
