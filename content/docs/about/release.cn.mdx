...
| 版本              |    发布时间    | 摘要                                                      | 地址                                                                                        |
|:----------------|:----------:|---------------------------------------------------------|-------------------------------------------------------------------------------------------|
| [v3.5.0](#v350) | 2025-05-31 | PG18 beta，421 扩展，监控升级，代码重构                              | [v3.5.0](https://github.com/pgsty/pigsty/releases/tag/v3.5.0)                             |
...
- 新增 Docker 参数：`docker_data`， 以及 `docker_storage_driver` by [#521](https://github.com/pgsty/pigsty/pull/521) by [@waitingsong]https://github.com/waitingsong)
...
- 修复 pg-backup 返回状态码：[#532](https://github.com/pgsty/pigsty/pull/532) by [@waitingsong]https://github.com/waitingsong)
- 在 `pg-tune-hugepage` 中设置只允许 PG 使用大页 [#527](https://github.com/pgsty/pigsty/pull/527) by [@waitingsong]https://github.com/waitingsong)
- 修复 `pg-role` 中的问题逻辑
...
- 新特性：为常见命令 `git`, `docker`, `systemctl` 添加自动补全支持 [#506](https://github.com/pgsty/pigsty/pull/506) [#507](https://github.com/pgsty/pigsty/pull/507) by [@waitingsong](https://github.com/waitingsong)
- 改进：改进 `pgbouncer` 配置模板的 `ignore_startup_parameters` 参数，忽略若干超时参数设置 [#488](https://github.com/pgsty/pigsty/pull/488) by [@waitingsong](https://github.com/waitingsong)
...
新增 12 个 PostgreSQL 扩展，可用总数达到 [404](https://pigsty.cc/ext/list/) 整。
...
- [`repo_url_packages`](https://pigsty.cc/docs/infra/param#repo_url_packages) 参数现在默认值为空数组，因为所有软件包现在都通过操作系统包管理器进行安装。
...
- [精简安装模式](https://pigsty.io/docs/install/slim/)：使用 `slim.yml` 进行最精简的 HA PGSQL 部署。
...
Pigsty v3 提供了史无前例的 [**333**](https://pigsty.cc/ext/list/) 个可用扩展插件。 包括 **121** 个扩展 [**RPM包**](https://pigsty.cc/ext/list/rpm/) 与 **133** 个 [**DEB包**](https://pigsty.cc/ext/list/deb/)，数量已经超过了 PGDG 官方仓库提供的扩展数量总和（135 RPM/ 109 DEB）。 而且，Pigsty 还将EL系统与Debian生态的独有PG扩展插件相互移植，实现了两大发行版的插件生态大对齐。
...
我们现在提供 Pigsty Pro [**专业版**](https://pigsty.cc/docs/price)，在开源版的功能基础上提供增值服务。专业版提供额外的功能模块：MSSQL，Oracle，Mongo，K8S，Victoria，Kafka，TigerBeetle 等……，并提供更广泛的 PG 大版本、操作系统、芯片架构的支持。 提供针对全系操作系统精准小版本定制的离线安装包，以及 EL7，Debian 11，Ubuntu 20.04 等过保老系统的支持；此外，专业版还提供内核可插拔定制服务，并对PolarDB PG 的原生部署、监控管控支持以满足“国产化”需要。
...
- 有在这些系统上运行需求的用户请考虑我们的 [订阅服务](https://pigsty.cc/docs/about/service/)
- 默认使用在线安装，不再提供离线软件包，从而解决操作系统小版本兼容性问题。
- `bootstrap` 过程现在不再询问是否下载离线安装包，但如果 `/tmp/pkg.tgz` 存在，仍然会自动使用离线安装包。
- 有离线安装需求请自行制作离线软件包或考虑我们的 [订阅服务](https://pigsty.cc/docs/about/service/)
...
- 参数强化：`node_packages` 与 `pg_extensions` 参数中指定的软件包在安装时会升级至最新版本， `node_packages` 中现在默认值变为 `[openssh-server`]，帮助修复 [OpenSSH CVE](https://pigsty.cc/blog/db/cve-2024-6387/)
...
Pigsty 重新编译了所有 PostgreSQL 扩展插件，PostgreSQL 扩展插件的最新版本，可用的 333 个扩展插件请参考 [**扩展列表**](https://pigsty.cc/ext/list/)

**新应用**

Pigsty 现在提供开箱即用的 Dify 与 Odoo 两款使用 PostgreSQL 软件的 Docker Compose 模板：

- [Dify](https://pigsty.cc/docs/software/dify)： AI智能体工作流编排与 LLMOps，使用 PostgreSQL 作为元数据库，PGVector 作为向量存储。
- [Odoo](https://pigsty.cc/docs/software/odoo)： 企业级开源 ERP 系统，使用 PostgreSQL 作为底层数据库。
...
- 通过 `node_packages` 中的默认值 `[openssh-server`]，[CVE-2024-6387](https://pigsty.cc/blog/db/cve-2024-6387/) 可以在 Pigsty 安装过程中被自动修复。
