...
Pigsty provides a **declarative** interface: Describe everything in a [config](/docs/config/inventory) file, and Pigsty operates it to the desired state with idempotent [playbooks](/docs/admin/playbook/). It works like Kubernetes CRDs & Operators but for databases and infrastructures on any nodes: bare metal or virtual machines.
