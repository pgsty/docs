...
You can define business [**Database**](/pgsql/db) and [**User**](/pgsql/user). These [config templates](/docs/config/template) has some pre-defined examples for your reference.
...
<Callout title="Use real domain names and certs" type="info">
    ...
    Just replace `domain` entries in [`infra_portal`](/infra/param#infra_portal), and use [`make cert`](/docs/admin/cert) to apply for real certs for free
