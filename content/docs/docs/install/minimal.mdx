...
There’s no [**`INFRA`**](/infra/) modules, no monitoring, no [**local repo**](/docs/admin/repo) Just [**`ETCD`**](/etcd/) & [**`PGSQL`**](/pgsql/) and partial of [**`NODE`**](/node/)
...
| haproxy | *OPTIONAL* | Auto-routing [**service**](/pgsql/service/) |
