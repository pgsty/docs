...
- [Identity](https://pigsty.io/docs/pgsql/concept/#identity-parameters): Parameters used for describing a PostgreSQL cluster
...
- [Major Version](https://pigsty.io/docs/pgsql/config/#major-version): Create postgres cluster with different major version
...
To add a physical replica, you can assign a new instance to `pg-test` with [`pg_role`](https://pigsty.io/docs/pgsql/param/#pg_role) set to `replica`
...
To add an offline instance, assign a new instance with [`pg_role`](https://pigsty.io/docs/pgsql/param/#pg_role) set to `offline`.
...
You can have ad hoc access control offline with [`pg_default_hba_rules`](https://pigsty.io/docs/pgsql/param/#pg_default_hba_rules) and [`pg_hba_rules`](https://pigsty.io/docs/pgsql/param/#pg_hba_rules). It will apply to the offline instance and any instances with [`pg_offline_query`](https://pigsty.io/docs/pgsql/param/#pg_offline_query) flag.

------

## Sync Standby

Pigsty uses asynchronous stream replication by default. Which may have a small replication lag. (10KB / 10ms). A small window of data loss may occur when the primary fails (can be controlled with [`pg_rpo`](https://pigsty.io/docs/pgsql/param/#pg_rpo).), but it is acceptable for most scenarios.
...
To enable sync standby mode, you can simply use `crit.yml` template in [`pg_conf`](https://pigsty.io/docs/pgsql/param/#pg_conf)
...
When [sync standby](https://pigsty.io/docs/pgsql/config/#sync-standby) is enabled, PostgreSQL will pick one replica as the standby instance, and all other replicas as candidates. Primary will wait until the standby instance flushes to disk before a commit is confirmed, and the standby instance will always have the latest data without any lags.
...
You can clone an existing cluster and create a [standby cluster](https://pigsty.io/docs/pgsql/config/#standby-cluster), which can be used for migration, horizontal split, multi-az deployment, or disaster recovery.

A standby cluster’s definition is just the same as any other normal cluster, except there’s a [`pg_upstream`](https://pigsty.io/docs/pgsql/param/#pg_upstream) defined on the primary instance.
...
Just make sure that the [`pg_upstream`](https://pigsty.io/docs/pgsql/param/#pg_upstream) parameter is configured on the primary of the backup cluster to pull backups from the original upstream automatically.
...
You can also [configure](https://pigsty.io/docs/pgsql/admin/#config-cluster) a replication delay on the existing [standby cluster](https://pigsty.io/docs/pgsql/config/#standby-cluster).
...
- [`pg_mode`](https://pigsty.io/docs/pgsql/param/#pg_mode) has to be set to `citus` instead of default `pgsql`
- [`pg_shard`](https://pigsty.io/docs/pgsql/param/#pg_shard) & [`pg_group`](https://pigsty.io/docs/pgsql/param/#pg_group) has to be defined on each sharding cluster
- [`patroni_primary_db`](https://pigsty.io/docs/pgsql/param/#pg_primary_db) has to be defined to specify the database to be managed
- [`pg_dbsu_password`](https://pigsty.io/docs/pgsql/param/#pg_dbsu_password) has to be set to a non-empty string plain password if you want to use the [`pg_dbsu`](https://pigsty.io/docs/pgsql/param/#pg_dbsu) `postgres` rather than default [`pg_admin_username`](https://pigsty.io/docs/pgsql/param/#pg_admin_username) to perform admin commands
...
Since some extensions are not available on PG 12,13,14,16, you may have to change [`pg_extensions`](https://pigsty.io/docs/pgsql/param/#pg_extensions) and [`pg_libs`](https://pigsty.io/docs/pgsql/param/#pg_libs) to fit your needs.
