...
Beware, when using the [`pgsql.yml`](https://pigsty.io/docs/pgsql/playbook/#pgsqlyml) and [`pgsql-rm.yml`](https://pigsty.io/docs/pgsql/playbook/#pgsql-rmyml) playbooks, it can pose a risk of accidentally deleting databases if misused!
...
To prevent accidental deletions, the [PGSQL](https://pigsty.io/docs/pgsql/) module offers a safeguard option controlled by the following two parameters:

- [`pg_safeguard`](https://pigsty.io/docs/pgsql/param/#pg_safeguard) is set to `false` by default: do not prevent purging by default.
- [`pg_clean`](https://pigsty.io/docs/pgsql/param/#pg_clean) is set to `true` by default, meaning it will clean existing instances.

**Effects on the init playbook**

When meeting a running instance with the same config during the execution of the [`pgsql.yml`](https://pigsty.io/docs/pgsql/playbook/#pgsqlyml) playbook:
...
- If [`pg_safeguard`](https://pigsty.io/docs/pgsql/param/#pg_safeguard) is enabled, the playbook will abort to avoid purging the running instance.

- If the safeguard is disabled, it will further decide whether to remove the existing instance according to the value of
...
When meeting a running instance with the same config during the execution of the [`pgsql-rm.yml`](https://pigsty.io/docs/pgsql/playbook/#pgsql-rmyml) playbook:
...
- If the safeguard is disabled, it purges the running instance and will further decide whether to remove the existing data along with the instance according to the value of
...
- you may have to run [`Reload HBARule`](https://pigsty.io/docs/pgsql/admin/#reload-hbarule) and [`Append Replica`](https://pigsty.io/docs/pgsql/admin/#append-replica) after replica init.
- The wrap script `pgsql-add` will do this, check SOP: [Add Instance](https://pigsty.io/docs/pgsql/admin/#add-instance) for details.
...
- But you should [reload service](https://pigsty.io/docs/pgsql/admin/#reload-service) in time to ensure the consistency between the environment and the config inventory.
...
Check admin SOP: [`Create User`](https://pigsty.io/docs/pgsql/admin/#create-user)
...
Check admin SOP: [`Create Database`](https://pigsty.io/docs/pgsql/admin/#create-database)
...
Check admin SOP: [`Monitor Postgres`](https://pigsty.io/docs/pgsql/monitor/#monitor-rds)
...
Check admin SOP: [Migration](https://pigsty.io/docs/pgsql/migration/)
