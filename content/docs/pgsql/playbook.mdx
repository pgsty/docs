...
- [`pg_safeguard`](/docs/pgsql/param#pg_safeguard) is set to `false` by default: do not prevent purging by default.
- [`pg_clean`](/docs/pgsql/param#pg_clean) is set to `true` by default, meaning it will clean existing instances.
...
- If [`pg_safeguard`](/docs/pgsql/param#pg_safeguard) is enabled, the playbook will abort to avoid purging the running instance.
