...
- [`pgsql-db.yml`](/docs/pgsql/playbook/#pgsql-dbyml) : Add a new business database to existing PostgreSQL cluster
...
Setting the [`pg_safeguard`](/docs/pgsql/param#pg_safeguard) parameter to `true` will stop the [`pgsql-rm.yml`](#pgsql-rmyml) from running.
...
<Callout title="Init Primary before Replicas" type="warning">
    ...
    - The wrap script `pgsql-add` will do this, check SOP: [Add Instance](/docs/pgsql/admin#append-replica) for details.
...
<Callout title="Init Upstream cluster before Standby Clusters" type="warning">
    ...
    - If you are initializing a [standby cluster](/docs/pgsql/config#standby-cluster), you should make sure the upstream cluster is already initialized.
