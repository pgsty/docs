...
Beware, when using the [`pgsql.yml`](/docs/pgsql/playbook/#pgsqlyml) and [`pgsql-rm.yml`](/docs/pgsql/playbook/#pgsql-rmyml) playbooks, it can pose a risk of accidentally deleting databases if misused!
...
To prevent accidental deletions, the [PGSQL](/docs/pgsql/) module offers a safeguard option controlled by the following two parameters:
...
When meeting a running instance with the same config during the execution of the [`pgsql.yml`](/docs/pgsql/playbook/#pgsqlyml) playbook:
...
When meeting a running instance with the same config during the execution of the [`pgsql-rm.yml`](/docs/pgsql/playbook/#pgsql-rmyml) playbook:
...
- you may have to run [`Reload HBARule`](/docs/pgsql/admin/#reload-hbarule) and [`Append Replica`](/docs/pgsql/admin/#append-replica) after replica init.
- The wrap script `pgsql-add` will do this, check SOP: [Add Instance](/docs/pgsql/admin/#add-instance) for details.
...
- But you should [reload service](/docs/pgsql/admin/#reload-service) in time to ensure the consistency between the environment and the config inventory.
...
Check admin SOP: [`Create User`](/docs/pgsql/admin/#create-user)
...
Check admin SOP: [`Create Database`](/docs/pgsql/admin/#create-database)
...
Check admin SOP: [`Monitor Postgres`](/docs/pgsql/monitor/#monitor-rds)
...
Check admin SOP: [Migration](/docs/pgsql/migration/)
