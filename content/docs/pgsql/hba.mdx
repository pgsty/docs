...
- [`pg_hba_rules`](/docs/pgsql/param#pg_hba_rules): postgres ad-hoc hba rules
- [`pg_default_hba_rules`](/docs/pgsql/param#pg_default_hba_rules): postgres default hba rules
- [`pgb_hba_rules`](/docs/pgsql/param#pgb_hba_rules): pgbouncer ad-hoc hba rules
- [`pgb_default_hba_rules`](/docs/pgsql/param#pgb_default_hba_rules): pgbouncer default hba rules
...
An HBA Rule is installed when the instance’s [`pg_role`](/docs/pgsql/param#pg_role) is the same as the `role`.

HBA Rule with `role: common` will be installed on all instances.

HBA Rule with `role: offline` will be installed on instances with [`pg_role`](/docs/pgsql/param#pg_role) = `offline` or [`pg_offline_query`](/docs/pgsql/param#pg_offline_query) = `true`.
...
- `auth`: how
  ...
  - `pwd`: use `md5` or `scram-sha-256` password auth according to [`pg_pwd_enc`](/docs/pgsql/param#pg_pwd_enc)
...
- `user`: who
  - `all`: all users
  - `${dbsu}`: database superuser specified by [`pg_dbsu`](/docs/pgsql/param#pg_dbsu)
  - `${repl}`: replication user specified by [`pg_replication_username`](/docs/pgsql/param#pg_replication_username)
  - `${admin}`: admin user specified by [`pg_admin_username`](/docs/pgsql/param#pg_admin_username)
  - `${monitor}`: monitor user specified by [`pg_monitor_username`](/docs/pgsql/param#pg_monitor_username)
  - ad hoc users & roles.
...
- [`pg_hba_rules`](/docs/pgsql/param#pg_hba_rules): postgres HBA rules for the cluster
- [`pgb_hba_rules`](/docs/pgsql/param#pgb_hba_rules): pgbouncer HBA rules for the cluster
