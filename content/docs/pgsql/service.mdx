...
In Pigsty, services are implemented using [haproxy](/node/param#haproxy) on [nodes](/node/), differentiated by different ports on the host node.
...
The boundary of Pigsty’s service delivery stops at the cluster’s HAProxy. Users can access these load balancers in various ways. Please refer to [Access Service](/pgsql/service#access-service).

All services are declared through configuration files. For instance, the default PostgreSQL service is defined by the [`pg_default_services`](/pgsql/param#pg_default_services) parameter:
...
You can also define new service in [`pg_services`](/pgsql/param#pg_services). And `pg_default_services` and `pg_services` are both arrays of [Service Definition](/pgsql/service/#define-service).
...
The default services are defined in [`pg_default_services`](/pgsql/param#pg_default_services).

While you can define your extra PostgreSQL services with [`pg_services`](/pgsql/param#pg_services) @ the global or cluster level.
...
When cluster membership has changed, such as append / remove replicas, switchover/failover, or adjust relative weight, You have to [reload service](/pgsql/admin#reload-service) to make the changes take effect.
...
When defining a service, if `svc.dest='default'`, this parameter [`pg_default_service_dest`](/pgsql/param#pg_default_service_dest) will be used as the default value. `pgbouncer` is used by default, you can use `postgres` instead, so the default primary & replica service will bypass pgbouncer and route traffic to postgres directly

If you don’t need connection pooling at all, you can change [`pg_default_service_dest`](/pgsql/param#pg_default_service_dest) to `postgres`, and remove `default` and `offline` services.
...
To do so, you will have to override the default service definition with [`pg_default_services`](/pgsql/param#pg_default_services) and set [`pg_service_provider`](/pgsql/param#pg_service_provider) to the proxy group name.
...
- **Synchronous replica service (standby)**: A read-only service with no replication delay, handled by [synchronous standby](/pgsql/config#sync-standby)/primary for read queries.
- **Delayed replica service (delayed)**: Accesses older data from the same cluster from a certain time ago, handled by [delayed replicas](/pgsql/config#delayed-cluster).
...
|--------------------------------------------|------|-------------------------------------------------------|
| [primary](/pgsql/service/#primary-service) | 5433 | pgbouncer read/write, connect to primary 5432 or 6432 |
| [replica](/pgsql/service/#replica-service) | 5434 | pgbouncer read-only, connect to replicas 5432/6432    |
| [default](/pgsql/service/#default-service) | 5436 | admin or direct access to primary                     |
| [offline](/pgsql/service/#offline-service) | 5438 | OLAP, ETL, personal user, interactive queries         |
...
[![pigsty-ha.png](https://pigsty.io/img/pigsty/ha.png)](/docs/feat/ha/)
...
It is responsible for routing traffic to different instances, check [Access Services](#access-service) for details.
...
It will route traffic to the primary instance, depending on [`pg_default_service_dest`](/pgsql/param/#pg_default_service_dest):
...
The replica service will route traffic to Pgbouncer or postgres depending on [`pg_default_service_dest`](/pgsql/param/#pg_default_service_dest), just like the [primary service](/pgsql/service/#primary-service).

```yaml
- { name: replica ,port: 5434 ,dest: default  ,check: /read-only ,selector: "[]" , backup: "[? pg_role == `primary` || pg_role == `offline` ]" }
```

The `replica` service traffic will try to use common pg instances with [`pg_role`](/pgsql/param/#pg_role) = `replica` to alleviate the load on the `primary` instance as much as possible. It will try NOT to use instances with [`pg_role`](/pgsql/param/#pg_role) = `offline` to avoid mixing OLAP & OLTP queries as much as possible.
...
It is quite like the primary service, except it will always bypass pgbouncer, regardless of [`pg_default_service_dest`](/pgsql/param/#pg_default_service_dest). Which is useful for administration connection, ETL writes, CDC changing data capture, etc…
...
Which could be a [`pg_role`](/pgsql/param/#pg_role) = `offline` instance, or a [`pg_offline_query`](/pgsql/param/#pg_offline_query) flagged instance.
...
Pigsty exposes [service](/pgsql/service/#service-overview) with haproxy. Which is enabled on all nodes by default.
...
![pigsty-access.jpg](https://pigsty.io/img/pigsty/access.jpg)
