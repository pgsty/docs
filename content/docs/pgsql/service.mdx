...
The boundary of Pigsty’s service delivery stops at the cluster’s HAProxy. Users can access these load balancers in various ways. Please refer to [Access Service](/docs/concept/svc/#access-service).
...
You can also define new service in [`pg_services`](/docs/pgsql/param#pg_services). And `pg_default_services` 与 `pg_services` are both array of [Service Definition](/docs/pgsql/svc/#define-service).
