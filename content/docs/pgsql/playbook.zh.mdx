...
```
```yaml
pg_backup                : # 删除备份仓库（使用 `pg_rm_bkup=false` 禁用）
...
## `pgsql-user.yml` [#pgsql-useryml]

playbook [`pgsql-user.yml`](https://github.com/pgsty/pigsty/blob/main/pgsql-user.yml) 可以向现有 PostgreSQL 集群添加新业务用户。

查看管理 SOP：[`创建用户`](/zh/pgsql/admin#create-user)

------

## `pgsql-db.yml` [#pgsql-dbyml]

playbook [`pgsql-db.yml`](https://github.com/pgsty/pigsty/blob/main/pgsql-db.yml) 可以向现有 PostgreSQL 集群添加新业务数据库。

查看管理 SOP：[`创建数据库`](/zh/pgsql/admin#create-database)

------

## `pgsql-monitor.yml` [#pgsql-monitoryml]

playbook [`pgsql-monitor.yml`](https://github.com/pgsty/pigsty/blob/main/pgsql-monitor.yml) 可以使用本地导出器监控远程 postgres 实例。

查看管理 SOP：[`监控 Postgres`](/zh/pgsql/monitor/#monitor-rds)

------

## `pgsql-migration.yml` [#pgsql-migrationyml]

playbook [`pgsql-migration.yml`](https://github.com/pgsty/pigsty/blob/main/pgsql-migration.yml) 可以为现有 PostgreSQL 集群生成迁移手册和脚本。

查看管理 SOP：[迁移](/zh/pgsql/migration/)