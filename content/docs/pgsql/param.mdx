...
These default values are two aliases that are translated through [**alias mapping**](/pgsql/ext/pkg) into the main RPM/DEB package names for the current PG major version, as well as version-independent common components (such as Patroni, PgBackrest, etc.)
