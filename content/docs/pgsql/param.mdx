...
Here are some common parameters used to identify PGSQL [entities](/docs/pgsql/cluster/#identity-parameters): instance, service, etc…
...
default values: `[]`, each object in the array defines a [Database](/docs/pgsql/database/). Examples:
...
default values: `[]`, each object in the array defines a [**Service**](/docs/pgsql/service/#define-service). Examples:
...
For example `/usr/pgsql -> /usr/pgsql-17`. For more details, check [PGSQL File Structure](/docs/pgsql) for details.
...
Beware that if all your traffic is [accessed](/docs/pgsql/service/#access-service) via haproxy, there is no risk of brain split at all.
...
default value is four default services definition, which is explained in [PGSQL Service](/docs/pgsql/service/#define-service)
