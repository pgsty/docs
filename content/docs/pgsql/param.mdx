---
title: Parameter
description: customize postgres cluster with 115-parameter
icon: Variable
---
...
| [`PG_BUSINESS`](#pg_business)   | 11    | Postgres Business Object Definition - configuration for business users, databases, services, and authentication                                    |
...
| [`PG_BOOTSTRAP`](#pg_bootstrap) | 32    | Init a HA Postgres Cluster with Patroni - comprehensive cluster initialization including data directories, networking, and high availability setup |
...
| [`PG_SERVICE`](#pg_service)     | 13    | Exposing pg service, bind vip and register DNS - service exposure, load balancing, VIP management, and DNS registration                            |
| [`PG_EXPORTER`](#pg_exporter)   | 15    | Add Monitor for PGSQL Instance - monitoring setup with various exporters for metrics collection                                                    |
...
| [`pg_fs_main`](#pg_fs_main)                         | `path`     | C     | mountpoint/path for postgres main data, `/data` by default                    |
| [`pg_fs_bkup`](#pg_fs_bkup)                         | `path`     | C     | mountpoint/path for pg backup data, `/data/backup` by default                 |
...
| [`pg_rm_data`](#pg_rm_data)     | `bool` | G/C/A | remove postgres data during remove? true by default                 |
| [`pg_rm_bkup`](#pg_rm_bkup)     | `path` | G/C/A | remove pgbackrest backup during primary remove? true by default     |
| [`pg_rm_pkg`](#pg_rm_pkg)       | `path` | G/C/A | uninstall postgres packages during remove? true by default          |
...
If you wish to monitor remote postgres instances, define them in `pg_exporters` and load them with [`pgsql-monitor.yml`](/docs/pgsql/playbook/#pgsql-monitoryml) playbook.
...
These default values are two aliases that are translated through [**alias mapping**](/docs/pgsql/ext/pkg) into the main RPM/DEB package names for the current PG major version, as well as version-independent common components (such as Patroni, PgBackrest, etc.)
...
Pigsty has a built-in privileges based on the default role system, check [PGSQL Privileges](/docs/pgsql/acl#privileges) for details.
...
default value is four default services definitions, which are explained in [PGSQL Service](/docs/pgsql/service/#define-service)
...
These flags control the behavior of [`pgsql-rm.yml`](/docs/pgsql/playbook/#pgsql-rmyml) playbook, which is used to remove postgres instance or cluster.
...
It only performs on primary instance, and only when [`pgbackrest_enabled`](#pgbackrest_enabled) is `true`.
