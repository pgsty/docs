...
API Reference for [PGSQL](/docs/pgsql/) module:
...
Here are some common parameters used to identify PGSQL [entities](/docs/pgsql/concept/#identity-parameters): instance, service, etc…
...
Setting `pg_upstream` is set on `primary` instance indicate that this cluster is a [**Standby Cluster**](/docs/pgsql/config/#standby-cluster), and will receiving changes from upstream instance, thus the `primary` is actually a `standby leader`.
...
If you wish to monitoring remote postgres instances, define them in `pg_exporters` and load them with [`pgsql-monitor.yml`](/docs/pgsql/playbook/#pgsql-monitoryml) playbook.
...
Check [PGSQL Monitoring](/docs/pgsql/monitor/) for details.
...
default values: `[]`, each object in the array defines a [User/Role](/docs/pgsql/user/). Examples:
...
default values: `[]`, each object in the array defines a [Database](/docs/pgsql/db/). Examples:
...
default values: `[]`, each object in the array defines a [**Service**](/docs/pgsql/svc/#define-service). Examples:
...
Which are array of [hba](/docs/pgsql/hba/#define-hba) object, each hba object may look like
...
Similar to [`pg_hba_rules`](#pg_hba_rules), array of [hba](/docs/pgsql/hba/#define-hba) rule object, except this is for pgbouncer.
...
If enabled, [`pgsql.yml`](/docs/pgsql/playbook/#pgsqlyml) & [`pgsql-rm.yml`](/docs/pgsql/playbook/#pgsql-rmyml) will abort immediately if any postgres instance is running.
...
default value is `true`, it will purge existing postgres instance during [`pgsql.yml`](/docs/pgsql/playbook/#pgsqlyml) init. which makes the playbook idempotent.

if set to `false`, [`pgsql.yml`](/docs/pgsql/playbook/#pgsqlyml) will abort if there’s already a running postgres instance. and [`pgsql-rm.yml`](/docs/pgsql/playbook/#pgsql-rmyml) will NOT remove postgres data (only stop the server).
...
patroni log dir, `/pg/log/patroni` by default, which will be collected by [`promtail`](/docs/node/param/#promtail).
...
Beware that if all your traffic is [accessed](/docs/pgsql/svc/#access-service) via haproxy, there is no risk of brain split at all.
...
Patroni unsafe RESTAPI is protected by username/password by default, check [Config Cluster](/docs/pgsql/admin/#config-cluster) and [Patroni RESTAPI](https://patroni.readthedocs.io/en/latest/rest_api.html) for details.
...
default values: `oltp.yml`, but [configure](/docs/config/configure) procedure will set this value to `tiny.yml` if current node is a tiny node.
...
For existing clusters, you can [configure](/docs/pgsql/admin/#config-cluster) the `shared_preload_libraries` parameter of the cluster and apply it.
...
Check [delayed standby cluster](/docs/pgsql/config/#delayed-cluster) for details.
...
- [Default HBA Rules](/docs/pgsql/hba/#default-hba)
- Default Schemas
- Default Extensions
...
postgres default host-based authentication rules, array of [hba](/docs/pgsql/hba/#define-hba) rule object.

default value provides a fair enough security level for common scenarios, check [PGSQL Authentication](/docs/pgsql/hba/) for details.
...
pgbouncer default host-based authentication rules, array or [hba](/docs/pgsql/hba/#define-hba) rule object.
...
Check [PGSQL Backup & PITR](/docs/pgsql/pitr/) for details.
...
pgBackRest log dir, `/pg/log/pgbackrest` by default, which is referenced by [`promtail`](/docs/node/param/#promtail) the logging agent.
...
default values: `100`. you have to define it at instance vars, and [reload-service](/docs/pgsql/admin/#reload-service) to take effect.
...
When defining a [service](/docs/pgsql/svc/#define-service), if svc.dest=‘default’, this parameter will be used as the default value.
...
default value is four default services definition, which is explained in [PGSQL Service](/docs/pgsql/svc/#define-service)
