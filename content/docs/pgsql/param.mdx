---
title: Parameter
description: customize postgres cluster with 115 parameters
icon: Variable
---

API Reference for [PGSQL](https://pigsty.io/docs/pgsql/) module:
...
- [`PG_BOOTSTRAP`](#pg_bootstrap) : Init a HA Postgres Cluster with Patroni
...
- [`PG_SERVICE`](#pg_service) : Exposing pg service, bind vip and register DNS
- [`PG_EXPORTER`](#pg_exporter) : Add Monitor for PGSQL Instance

------

## Parameters

There are **115** parameters about the `PGSQL` module.

| Name                                                            | Comment                                                                       |
|-----------------------------------------------------------------|-------------------------------------------------------------------------------|
| [`pg_mode`](#pg_mode)                                           | pgsql cluster mode: pgsql,citus,mssql,polar,ivory,oracle,gpsql                |
| [`pg_cluster`](#pg_cluster)                                     | pgsql cluster name, REQUIRED identity parameter                               |
| [`pg_seq`](#pg_seq)                                             | pgsql instance seq number, REQUIRED identity parameter                        |
| [`pg_role`](#pg_role)                                           | pgsql role, REQUIRED, could be primary,replica,offline                        |
| [`pg_instances`](#pg_instances)                                 | define multiple pg instances on node in `{port:ins_vars}` format              |
| [`pg_upstream`](#pg_upstream)                                   | repl upstream ip addr for standby cluster or cascade replica                  |
| [`pg_shard`](#pg_shard)                                         | pgsql shard name, optional identity for sharding clusters                     |
| [`pg_group`](#pg_group)                                         | pgsql shard index number, optional identity for sharding clusters             |
| [`gp_role`](#gp_role)                                           | greenplum role of this cluster, could be master or segment                    |
| [`pg_exporters`](#pg_exporters)                                 | additional pg_exporters to monitor remote postgres instances                  |
| [`pg_offline_query`](#pg_offline_query)                         | set to true to enable offline query on this instance                          |
| [`pg_users`](#pg_users)                                         | postgres business users                                                       |
| [`pg_databases`](#pg_databases)                                 | postgres business databases                                                   |
| [`pg_services`](#pg_services)                                   | postgres business services                                                    |
| [`pg_hba_rules`](#pg_hba_rules)                                 | business hba rules for postgres                                               |
| [`pgb_hba_rules`](#pgb_hba_rules)                               | business hba rules for pgbouncer                                              |
| [`pg_replication_username`](#pg_replication_username)           | postgres replication username, `replicator` by default                        |
| [`pg_replication_password`](#pg_replication_password)           | postgres replication password, `DBUser.Replicator` by default                 |
| [`pg_admin_username`](#pg_admin_username)                       | postgres admin username, `dbuser_dba` by default                              |
| [`pg_admin_password`](#pg_admin_password)                       | postgres admin password in plain text, `DBUser.DBA` by default                |
| [`pg_monitor_username`](#pg_monitor_username)                   | postgres monitor username, `dbuser_monitor` by default                        |
| [`pg_monitor_password`](#pg_monitor_password)                   | postgres monitor password, `DBUser.Monitor` by default                        |
| [`pg_dbsu_password`](#pg_dbsu_password)                         | postgres dbsu password, empty string disable it by default                    |
| [`pg_dbsu`](#pg_dbsu)                                           | os dbsu name, postgres by default, better not change it                       |
| [`pg_dbsu_uid`](#pg_dbsu_uid)                                   | os dbsu uid and gid, 26 for default postgres users and groups                 |
| [`pg_dbsu_sudo`](#pg_dbsu_sudo)                                 | dbsu sudo privilege, none,limit,all,nopass. limit by default                  |
| [`pg_dbsu_home`](#pg_dbsu_home)                                 | postgresql home directory, `/var/lib/pgsql` by default                        |
| [`pg_dbsu_ssh_exchange`](#pg_dbsu_ssh_exchange)                 | exchange postgres dbsu ssh key among same pgsql cluster                       |
| [`pg_version`](#pg_version)                                     | postgres major version to be installed, 16 by default                         |
| [`pg_bin_dir`](#pg_bin_dir)                                     | postgres binary dir, `/usr/pgsql/bin` by default                              |
| [`pg_log_dir`](#pg_log_dir)                                     | postgres log dir, `/pg/log/postgres` by default                               |
| [`pg_packages`](#pg_packages)                                   | pg packages to be installed, `${pg_version}` will be replaced                 |
| [`pg_extensions`](#pg_extensions)                               | pg extensions to be installed, `${pg_version}` will be replaced               |
| [`pg_safeguard`](#pg_safeguard)                                 | prevent purging running postgres instance? false by default                   |
| [`pg_clean`](#pg_clean)                                         | purging existing postgres during pgsql init? true by default                  |
| [`pg_data`](#pg_data)                                           | postgres data directory, `/pg/data` by default                                |
| [`pg_fs_main`](#pg_fs_main)                                     | mountpoint/path for postgres main data, `/data` by default                    |
| [`pg_fs_bkup`](#pg_fs_bkup)                                     | mountpoint/path for pg backup data, `/data/backup` by default                 |
| [`pg_storage_type`](#pg_storage_type)                           | storage type for pg main data, SSD,HDD, SSD by default                        |
| [`pg_dummy_filesize`](#pg_dummy_filesize)                       | size of `/pg/dummy`, hold 64MB disk space for emergency use                   |
| [`pg_listen`](#pg_listen)                                       | postgres/pgbouncer listen addresses, comma separated list                     |
| [`pg_port`](#pg_port)                                           | postgres listen port, 5432 by default                                         |
| [`pg_localhost`](#pg_localhost)                                 | postgres unix socket dir for localhost connection                             |
| [`pg_namespace`](#pg_namespace)                                 | top level key namespace in etcd, used by patroni & vip                        |
| [`patroni_enabled`](#patroni_enabled)                           | if disabled, no postgres cluster will be created during init                  |
| [`patroni_mode`](#patroni_mode)                                 | patroni working mode: default,pause,remove                                    |
| [`patroni_port`](#patroni_port)                                 | patroni listen port, 8008 by default                                          |
| [`patroni_log_dir`](#patroni_log_dir)                           | patroni log dir, `/pg/log/patroni` by default                                 |
| [`patroni_ssl_enabled`](#patroni_ssl_enabled)                   | secure patroni RestAPI communications with SSL?                               |
| [`patroni_watchdog_mode`](#patroni_watchdog_mode)               | patroni watchdog mode: automatic,required,off. off by default                 |
| [`patroni_username`](#patroni_username)                         | patroni restapi username, `postgres` by default                               |
| [`patroni_password`](#patroni_password)                         | patroni restapi password, `Patroni.API` by default                            |
| [`pg_primary_db`](#pg_primary_db)                               | primary database name, used by citus,etc… ,postgres by default                |
| [`pg_parameters`](#pg_parameters)                               | extra parameters in postgresql.auto.conf                                      |
| [`pg_files`](#pg_files)                                         | extra files to be copied to postgres data directory (e.g. license)            |
| [`pg_conf`](#pg_conf)                                           | config template: oltp,olap,crit,tiny. `oltp.yml` by default                   |
| [`pg_max_conn`](#pg_max_conn)                                   | postgres max connections, `auto` will use recommended value                   |
| [`pg_shared_buffer_ratio`](#pg_shared_buffer_ratio)             | postgres shared buffer memory ratio, 0.25 by default, 0.1~0.4                 |
| [`pg_rto`](#pg_rto)                                             | recovery time objective in seconds, `30s` by default                          |
| [`pg_rpo`](#pg_rpo)                                             | recovery point objective in bytes, `1MiB` at most by default                  |
| [`pg_libs`](#pg_libs)                                           | preloaded libraries, `timescaledb,pg_stat_statements,auto_explain` by default |
| [`pg_delay`](#pg_delay)                                         | replication apply delay for standby cluster leader                            |
| [`pg_checksum`](#pg_checksum)                                   | enable data checksum for postgres cluster?                                    |
| [`pg_pwd_enc`](#pg_pwd_enc)                                     | passwords encryption algorithm: md5,scram-sha-256                             |
| [`pg_encoding`](#pg_encoding)                                   | database cluster encoding, `UTF8` by default                                  |
| [`pg_locale`](#pg_locale)                                       | database cluster local, `C` by default                                        |
| [`pg_lc_collate`](#pg_lc_collate)                               | database cluster collate, `C` by default                                      |
| [`pg_lc_ctype`](#pg_lc_ctype)                                   | database character type, `en_US.UTF8` by default                              |
| [`pgbouncer_enabled`](#pgbouncer_enabled)                       | if disabled, pgbouncer will not be launched on pgsql host                     |
| [`pgbouncer_port`](#pgbouncer_port)                             | pgbouncer listen port, 6432 by default                                        |
| [`pgbouncer_log_dir`](#pgbouncer_log_dir)                       | pgbouncer log dir, `/pg/log/pgbouncer` by default                             |
| [`pgbouncer_auth_query`](#pgbouncer_auth_query)                 | query postgres to retrieve unlisted business users?                           |
| [`pgbouncer_poolmode`](#pgbouncer_poolmode)                     | pooling mode: transaction,session,statement, transaction by default           |
| [`pgbouncer_sslmode`](#pgbouncer_sslmode)                       | pgbouncer client ssl mode, disable by default                                 |
| [`pg_provision`](#pg_provision)                                 | provision postgres cluster after bootstrap                                    |
| [`pg_init`](#pg_init)                                           | provision init script for cluster template, `pg-init` by default              |
| [`pg_default_roles`](#pg_default_roles)                         | default roles and users in postgres cluster                                   |
| [`pg_default_privileges`](#pg_default_privileges)               | default privileges when created by admin user                                 |
| [`pg_default_schemas`](#pg_default_schemas)                     | default schemas to be created                                                 |
| [`pg_default_extensions`](#pg_default_extensions)               | default extensions to be created                                              |
| [`pg_reload`](#pg_reload)                                       | reload postgres after hba changes                                             |
| [`pg_default_hba_rules`](#pg_default_hba_rules)                 | postgres default host-based authentication rules                              |
| [`pgb_default_hba_rules`](#pgb_default_hba_rules)               | pgbouncer default host-based authentication rules                             |
| [`pgbackrest_enabled`](#pgbackrest_enabled)                     | enable pgbackrest on pgsql host?                                              |
| [`pgbackrest_clean`](#pgbackrest_clean)                         | remove pg backup data during init?                                            |
| [`pgbackrest_log_dir`](#pgbackrest_log_dir)                     | pgbackrest log dir, `/pg/log/pgbackrest` by default                           |
| [`pgbackrest_method`](#pgbackrest_method)                       | pgbackrest repo method: local,minio,etc…                                      |
| [`pgbackrest_repo`](#pgbackrest_repo)                           | pgbackrest repo: https://pgbackrest.org/configuration.html#section-repository |
| [`pg_weight`](#pg_weight)                                       | relative load balance weight in service, 100 by default, 0-255                |
| [`pg_service_provider`](#pg_service_provider)                   | dedicate haproxy node group name, or empty string for local nodes by default  |
| [`pg_default_service_dest`](#pg_default_service_dest)           | default service destination if svc.dest=‘default’                             |
| [`pg_default_services`](#pg_default_services)                   | postgres default service definitions                                          |
| [`pg_vip_enabled`](#pg_vip_enabled)                             | enable a l2 vip for pgsql primary? false by default                           |
| [`pg_vip_address`](#pg_vip_address)                             | vip address in `<ipv4>/<mask>` format, require if vip is enabled              |
| [`pg_vip_interface`](#pg_vip_interface)                         | vip network interface to listen, eth0 by default                              |
| [`pg_dns_suffix`](#pg_dns_suffix)                               | pgsql dns suffix, ’’ by default                                               |
| [`pg_dns_target`](#pg_dns_target)                               | auto, primary, vip, none, or ad hoc ip                                        |
| [`pg_exporter_enabled`](#pg_exporter_enabled)                   | enable pg_exporter on pgsql hosts?                                            |
| [`pg_exporter_config`](#pg_exporter_config)                     | pg_exporter configuration file name                                           |
| [`pg_exporter_cache_ttls`](#pg_exporter_cache_ttls)             | pg_exporter collector ttl stage in seconds, ‘1,10,60,300’ by default          |
| [`pg_exporter_port`](#pg_exporter_port)                         | pg_exporter listen port, 9630 by default                                      |
| [`pg_exporter_params`](#pg_exporter_params)                     | extra url parameters for pg_exporter dsn                                      |
| [`pg_exporter_url`](#pg_exporter_url)                           | overwrite auto-generate pg dsn if specified                                   |
| [`pg_exporter_auto_discovery`](#pg_exporter_auto_discovery)     | enable auto database discovery? enabled by default                            |
| [`pg_exporter_exclude_database`](#pg_exporter_exclude_database) | csv of database that WILL NOT be monitored during auto-discovery              |
| [`pg_exporter_include_database`](#pg_exporter_include_database) | csv of database that WILL BE monitored during auto-discovery                  |
| [`pg_exporter_connect_timeout`](#pg_exporter_connect_timeout)   | pg_exporter connect timeout in ms, 200 by default                             |
| [`pg_exporter_options`](#pg_exporter_options)                   | overwrite extra options for pg_exporter                                       |
| [`pgbouncer_exporter_enabled`](#pgbouncer_exporter_enabled)     | enable pgbouncer_exporter on pgsql hosts?                                     |
| [`pgbouncer_exporter_port`](#pgbouncer_exporter_port)           | pgbouncer_exporter listen port, 9631 by default                               |
| [`pgbouncer_exporter_url`](#pgbouncer_exporter_url)             | overwrite auto-generate pgbouncer dsn if specified                            |
| [`pgbouncer_exporter_options`](#pgbouncer_exporter_options)     | overwrite extra options for pgbouncer_exporter                                |
| [`pgbackrest_exporter_enabled`](#pgbackrest_exporter_enabled)   | enable pgbackrest_exporter on pgsql hosts?                                    |
| [`pgbackrest_exporter_port`](#pgbackrest_exporter_port)         | pgbackrest_exporter listen port, 9854 by default                              |
| [`pgbackrest_exporter_options`](#pgbackrest_exporter_options)   | extra cli args for pgbackrest_exporter                                        |

------

## `PG_ID`

Here are some common parameters used to identify PGSQL [entities](https://pigsty.io/docs/pgsql/concept/#identity-parameters): instance, service, etc…
...
Setting `pg_upstream` is set on `primary` instance indicate that this cluster is a [**Standby Cluster**](https://pigsty.io/docs/pgsql/config/#standby-cluster), and will receiving changes from upstream instance, thus the `primary` is actually a `standby leader`.
...
If you wish to monitoring remote postgres instances, define them in `pg_exporters` and load them with [`pgsql-monitor.yml`](https://pigsty.io/docs/pgsql/playbook/#pgsql-monitoryml) playbook.
...
Check [PGSQL Monitoring](https://pigsty.io/docs/pgsql/monitor/) for details.
...
[Default Database Users](https://pigsty.io/docs/pgsql/acl/#default-users):
...
default values: `[]`, each object in the array defines a [User/Role](https://pigsty.io/docs/pgsql/user/). Examples:
...
default values: `[]`, each object in the array defines a [Database](https://pigsty.io/docs/pgsql/db/). Examples:
...
default values: `[]`, each object in the array defines a [**Service**](https://pigsty.io/docs/pgsql/svc/#define-service). Examples:
...
Which are array of [hba](https://pigsty.io/docs/pgsql/hba/#define-hba) object, each hba object may look like
...
Similar to [`pg_hba_rules`](#pg_hba_rules), array of [hba](https://pigsty.io/docs/pgsql/hba/#define-hba) rule object, except this is for pgbouncer.
...
For example `/usr/pgsql -> /usr/pgsql-17`. For more details, check [PGSQL File Structure](https://pigsty.io/docs/reference/fhs/#postgres-fhs) for details.
...
These default values are two aliases that are translated through [**alias mapping**](https://pigsty.io/ext/usage/download/#alias-mapping) into the main RPM/DEB package names for the current PG major version, as well as version-independent common components (such as Patroni, PgBackrest, etc.)
...
The full extension list can be found in [`roles/node_id/vars`](https://github.com/pgsty/pigsty/blob/main/roles/node_id/vars/) and listed in [Extension List](https://pigsty.io/ext/list/).
...
If enabled, [`pgsql.yml`](https://pigsty.io/docs/pgsql/playbook/#pgsqlyml) & [`pgsql-rm.yml`](https://pigsty.io/docs/pgsql/playbook/#pgsql-rmyml) will abort immediately if any postgres instance is running.
...
default value is `true`, it will purge existing postgres instance during [`pgsql.yml`](https://pigsty.io/docs/pgsql/playbook/#pgsqlyml) init. which makes the playbook idempotent.

if set to `false`, [`pgsql.yml`](https://pigsty.io/docs/pgsql/playbook/#pgsqlyml) will abort if there’s already a running postgres instance. and [`pgsql-rm.yml`](https://pigsty.io/docs/pgsql/playbook/#pgsql-rmyml) will NOT remove postgres data (only stop the server).
...
Check [PGSQL File Structure](https://pigsty.io/docs/reference/fhs/) for details.
...
It’s recommended to use NVME SSD for postgres main data storage, Pigsty is optimized for SSD storage by default. But HDD is also supported, you can change [`pg_storage_type`](#pg_storage_type) to `HDD` to optimize for HDD storage.
...
patroni log dir, `/pg/log/patroni` by default, which will be collected by [`promtail`](https://pigsty.io/docs/node/param/#promtail).
...
Beware that if all your traffic is [accessed](https://pigsty.io/docs/pgsql/svc/#access-service) via haproxy, there is no risk of brain split at all.
...
Patroni unsafe RESTAPI is protected by username/password by default, check [Config Cluster](https://pigsty.io/docs/pgsql/admin/#config-cluster) and [Patroni RESTAPI](https://patroni.readthedocs.io/en/latest/rest_api.html) for details.
...
default values: `oltp.yml`, but [configure](https://pigsty.io/docs/setup/install/#configure) procedure will set this value to `tiny.yml` if current node is a tiny node.
...
For existing clusters, you can [configure](https://pigsty.io/docs/pgsql/admin/#config-cluster) the `shared_preload_libraries` parameter of the cluster and apply it.
...
Check [delayed standby cluster](https://pigsty.io/docs/pgsql/config/#delayed-cluster) for details.
...
- [Default Roles](https://pigsty.io/docs/pgsql/acl/#default-roles)
- [Default Users](https://pigsty.io/docs/pgsql/acl/#default-users)
- [Default Privileges](https://pigsty.io/docs/pgsql/acl/#privileges)
- [Default HBA Rules](https://pigsty.io/docs/pgsql/hba/#default-hba)
- Default Schemas
- Default Extensions
...
Pigsty has a built-in role system, check [PGSQL Access Control](https://pigsty.io/docs/pgsql/acl/#role-system) for details.
...
Pigsty has a built-in privileges base on default role system, check [PGSQL Privileges](https://pigsty.io/docs/pgsql/acl/#privileges) for details.
...
The only 3rd party extension is [`pg_repack`](https://pigsty.io/ext/admin/pg_repack/), which is important for database maintenance, all other extensions are built-in postgres contrib extensions.
...
postgres default host-based authentication rules, array of [hba](https://pigsty.io/docs/pgsql/hba/#define-hba) rule object.

default value provides a fair enough security level for common scenarios, check [PGSQL Authentication](https://pigsty.io/docs/pgsql/hba/) for details.
...
pgbouncer default host-based authentication rules, array or [hba](https://pigsty.io/docs/pgsql/hba/#define-hba) rule object.
...
Check [PGSQL Backup & PITR](https://pigsty.io/docs/pgsql/pitr/) for details.
...
pgBackRest log dir, `/pg/log/pgbackrest` by default, which is referenced by [`promtail`](https://pigsty.io/docs/node/param/#promtail) the logging agent.
...
default values: `100`. you have to define it at instance vars, and [reload-service](https://pigsty.io/docs/pgsql/admin/#reload-service) to take effect.
...
When defining a [service](https://pigsty.io/docs/pgsql/svc/#define-service), if svc.dest=‘default’, this parameter will be used as the default value.
...
default value is four default services definition, which is explained in [PGSQL Service](https://pigsty.io/docs/pgsql/svc/#define-service)
