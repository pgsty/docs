---
title: Repository
description: Backup storage repository for PostgreSQL
icon: DatabaseBackup
---



You can to configure **WHERE** to store the backups by specifying the [`pgbackrest_repo`](/pgsql/param/#pgbackrest_repo) parameter.
You can define multiple repo there, and Pigsty will pick it according to the value of [`pgbackrest_method`](/pgsql/param/#pgbackrest_method).
...
The default backup policy is defined in the [`pgbackrest_repo`](/pgsql/param/#pgbackrest_repo) parameter, change them on demand.
...
This gives you a guaranteed recovery window of at least last 24 hours. Check [backup policy](/pgsql/backup/policy) for details.
...
You can also use other services as backup repo, check [pgbackrest documentation](https://pgbackrest.org/user-guide.html) for details:

- [S3-Compatible Object Store](https://pgbackrest.org/user-guide.html#s3-support)
- [Azure-Compatible Object Store](https://pgbackrest.org/user-guide.html#azure-support)
- [GCS-Compatible Object Store](https://pgbackrest.org/user-guide.html#gcs-support)
...
You can even specify a [repo target time](https://pgbackrest.org/user-guide.html#sftp-support#repo-target-time) to get a snapshot of object storage.

You can enable MinIO versioning by adding `versioning` flag to the [`minio_buckets`](/minio/param#minio_buckets):
...
You can enable MinIO locking feature by adding `lock` flag to the [`minio_buckets`](/minio/param#minio_buckets):
...
If you don't have one, Pigsty has built-in [MinIO](/minio) support.
