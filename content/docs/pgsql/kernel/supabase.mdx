...
You can run the latest PostgreSQL 17(,16,15) kernels, (supabase is using the 15 currently), alone with [421](/ext/list/) PostgreSQL extensions out-of-the-box. Run on [mainstream](/docs/prepare/linux/) Linus OS distros
with production grade [HA](/docs/feat/ha/) [PostgreSQL](/pgsql/), [MinIO](/minio/), Prometheus & Grafana Stack for observability, and Nginx for reverse proxy.
...
First, download & [install](/docs/install/) pigsty as usual, with the `supa` config template:
...
-  [**Hardware**](/docs/prepare/hardware#node): necessary VM/BM resources, one node at least, 3-4 are recommended for HA.
-  [**Linux OS**](/docs/prepare/linux): Linux x86_64 server with fresh installed Linux, [check compatible distro](/docs/prepare/linux)
-  [**Network**](/docs/prepare/hardware#network): Static IPv4 address which can be used as node identity
-  [**Admin User**](/docs/prepare/admin#user): nopass ssh & sudo are recommended for admin user
-  [**Config Template**](/docs/config/template): Use the [`supabase`](/docs/config/template) config template, if you don’t know how to manually configure pigsty
...
You can [refer to the certbot tutorial](/docs/admin/cert) to apply for a free HTTPS certificate. Here we assume your custom domain name is: `supa.pigsty.cc`, then you should modify the `supa` domain in [`infra_portal`](/infra/param/#infra_portal) like this:
