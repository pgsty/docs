...
In case of hardware failures, failover to physical replica could be more efficient, as it can be done in seconds without data loss.
Whereas for data corruption scenarios or accidental data loss (whether bugs or human errors), Point-in-Time Recovery (PITR) is often more appropriate.
...
You can also acquire backup info from the monitoring system: [PGCAT Instance - Backup](https://demo.pigsty.io/d/pgcat-instance/pgcat-instance?from=now-1h&to=now&var-job=pgsql&var-ds=Prometheus&orgId=1&viewPanel=158) or the dedicate PGSQL PITR dashboard.
...
You can customize your backup policy with [`node_crontab`](/docs/node/param#node_crontab) and [`pgbackrest_repo`](/docs/pgsql/param#pgbackrest_repo)

- Schedule full or incr backup with [`node_crontab`](/docs/node/param#node_crontab)
- setup backup retention policy with [`pgbackrest_repo`](/docs/pgsql/param#pgbackrest_repo)
