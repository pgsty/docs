...
Refer to the [**Extensions Without DDL**](/pgsql/ext/config) section for more details.
...
If you’re using an extension that requires [**dynamic loading**](/pgsql/ext/config) (which modifies the `shared_preload_libraries` parameter), you need to first [**re-confnigure**](/pgsql/admin#config-cluster) the `shared_preload_libraries` parameter.
