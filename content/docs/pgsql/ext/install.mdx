...
When installing extensions, Pigsty uses the same [**alias mapping**](/docs/pgsql/ext/pkg) in the [**download**](/docs/pgsql/ext/download) section.
...
- **Local Repo**: You have configured using Pigsty’s local repo, and the extensions have already been [**downloaded**](/docs/pgsql/ext/download/) to the local repo.
...
For production environments, we recommend using Pigsty’s local software repo to manage and install extensions uniformly: First [**download**](/docs/pgsql/ext/download) extensions to the local repo, then install them from there.
...
When installing extensions, users can use [**extension aliases**](/docs/pgsql/ext/pkg) to specify extension.

The aliases will be [**translated**](/docs/pgsql/ext/pkg) to the current active PG major version and OS environment.

and translated to the corresponding RPM/DEB package names by [**alias translation mechanism**](/docs/pgsql/ext/pkg).

------

## Caveats

- There are two known conflictions:
  - [`citus`](/ext/citus/) and [`hydra`](/ext/hydra) are mutually exclusive, since hydra is a fork of citus columnar without renaming
  - Only install one from [`pg_duckdb`](/ext/pg_duckdb), [`pg_mooncake`](/ext/pg_mooncake), [`duckdb_fdw`](/ext/duckdb_fdw), they all using their own libduckdb
