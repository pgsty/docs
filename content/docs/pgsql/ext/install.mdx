...
When installing extensions, Pigsty uses the same [**alias mapping**](/pgsql/ext/pkg) in the [**download**](/pgsql/ext/download) section.
...
- **Local Repo**: You have configured using Pigsty’s local repo, and the extensions have already been [**downloaded**](/pgsql/ext/download/) to the local repo.
...
For production environments, we recommend using Pigsty’s local software repo to manage and install extensions uniformly: First [**download**](/pgsql/ext/download) extensions to the local repo, then install them from there.
...
When installing extensions, users can use [**extension aliases**](/pgsql/ext/pkg) to specify extension.

The aliases will be [**translated**](/pgsql/ext/pkg) to the current active PG major version and OS environment.

and translated to the corresponding RPM/DEB package names by [**alias translation mechanism**](/pgsql/ext/pkg).
