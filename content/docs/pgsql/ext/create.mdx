...
Extensions need to be [**installed**](/docs/pgsql/ext/install) first, some extension also require [**preloading**](/docs/pgsql/ext/config) before using.
...
- Alternatively, A dictionary that may contain the following fields can be used:
  - `name`: Extension name, **REQUIRED**, beware it may differ from the [**extension package name**](/docs/pgsql/ext/pkg).
...
But it's helpful to enlist them in the [config inventory](/docs/config/inventory) for bookkeeping purposes. (So if you want to fork this cluster, it include these extensions)
...
Newly create databases will inherit these extensions from `template1`, so you don't need to create them again.
...
|  [`wal2json`](/ext/wal2json)  | Changing data capture in JSON | extension [without DDL](/ext/list/attr#without-ddl), [install](/docs/pgsql/ext/install) means available |
...
[**`vector`**](/ext/vector) is a very popular extensions for RAG,
It is [**installed**](/docs/pgsql/ext/install) by default (in the `pgsql-main` [alias](/docs/pgsql/ext/pkg)) and created in the placeholder `meta` database in most [config templates](/docs/config/templates).

The [`wal2json`](/ext/wal2json) is another important extension for Chanaging Data Capture (CDC). It is [installed](/docs/pgsql/ext/install) by default, but it is an [**extension without DDL**](#extension-without-ddl),
...
If extension does not have SQL file, `CREATE EXTENSION` command is not needed.
