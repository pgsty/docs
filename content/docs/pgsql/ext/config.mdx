...
Note that [`pg_libs`](/docs/pgsql/param#pg_libs) only takes effect during [**cluster creation**](/docs/pgsql/admin#create-cluster). After creation, it becomes the initial value for the PostgreSQL parameter `shared_preload_libraries`.
