...
While most PostgreSQL extensions written in SQL can be [**directly enabled**](/docs/pgsql/ext/create) with `CREATE EXTENSION`, some extensions that use special postgres hook will require an extra step to **preload** them before using.
...
Most extension has one or more corresponding dynamic library (`.so`, `.dylib`, `.dll`), some of them require preloading before using.
Attempting to `CREATE` these EXTENSION without proper preloading will result in an error. 
...
To preloading an extension, add it to the [`shared_preload_libraries`](https://www.postgresql.org/docs/current/runtime-config-client.html#Rshared_preload_libraries) and restart the database server.
