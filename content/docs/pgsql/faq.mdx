...
> Use `crit.yml` template, or setting `pg_rpo` to `0`, or [config cluster](/docs/pgsql/admin/#config-cluster) with synchronous mode.

Consider using [Sync Standby](/docs/pgsql/config/#sync-standby) and [Quorum Comit](/docs/pgsql/config/#quorum-commit) to guarantee 0 data loss during failover.
...
Check [PGSQL Monitor](/docs/pgsql/monitor/) for details.
