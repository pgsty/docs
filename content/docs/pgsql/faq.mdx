...
This happens when you run `pgsql.yml` on a node with postgres running, and [`pg_clean`](https://pigsty.io/docs/pgsql/param/#pg_clean) is set to `false`.
...
If [`pg_safeguard`](https://pigsty.io/docs/pgsql/param/#pg_safeguard) is enabled, you can not remove the running pgsql instance with `bin/pgsql-rm` and `pgsql-rm.yml` playbook.
...
The [`pg_dummy_filesize`](https://pigsty.io/docs/pgsql/param/#pg_dummy_filesize) is set to `64MB` by default. Consider increasing it to `8GB` or larger in the production environment.
