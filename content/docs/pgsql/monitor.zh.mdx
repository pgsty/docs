...
监控系统将不再有主机/池化器/负载均衡器指标。但 PostgreSQL 指标和目录信息仍然可用。Pigsty 为此有两个专用仪表板：[PGRDS 集群](https://demo.pigsty.cc/d/pgrds-cluster) 和 [PGRDS 实例](https://demo.pigsty.cc/d/pgrds-instance)。概览和数据库级别仪表板被重用。由于 Pigsty 无法管理您的 RDS，您必须提前在目标数据库上[设置监控](/zh/pgsql/monitor/#monitor-setup)。
