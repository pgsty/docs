---
title: Monitor
description: Monitor existing PostgreSQL or RDS
icon: Telescope
---
...
- Battery-Include [dashboards](https://pigsty.io/docs/pgsql/dashboard/) for PostgreSQL and everything else

### **Metrics**

PostgreSQL’s metrics are defined by collector files: [`pg_exporter.yml`](https://github.com/pgsty/pigsty/blob/main/roles/pgsql/templates/pg_exporter.yml). Prometheus record rules and alert evaluation will further process it: [`files/prometheus/rules/pgsql.yml`](https://github.com/pgsty/pigsty/blob/main/files/prometheus/rules/pgsql.yml)
...
- [`pg_log_dir`](https://pigsty.io/docs/pgsql/param/#pg_log_dir) : postgres log dir, `/pg/log/postgres` by default
- [`pgbouncer_log_dir`](https://pigsty.io/docs/pgsql/param/#pgbouncer_log_dir) : pgbouncer log dir, `/pg/log/pgbouncer` by default
- [`patroni_log_dir`](https://pigsty.io/docs/pgsql/param/#patroni_log_dir) : patroni log dir, `/pg/log/patroni` by default
- [`pgbackrest_log_dir`](https://pigsty.io/docs/pgsql/param/#pgbackrest_log_dir) : pgbackrest log dir, `/pg/log/pgbackrest` by default
...
When the global flag [`patroni_ssl_enabled`](https://pigsty.io/docs/pgsql/param/#patroni_ssl_enabled) is set, the patroni target will be managed as `/etc/prometheus/targets/patroni/<ins>.yml` because it requires a different scrape endpoint (https).
...
Remote RDS targets are managed as `/etc/prometheus/targets/pgrds/<cls>.yml`. It will be created by the [`pgsql-monitor.yml`](https://pigsty.io/docs/pgsql/playbook/#pgsql-monitor) playbook or `bin/pgmon-add` script.
...
|        Item \ Level        |                              L1                              |                              L2                              |               L3                |
| :------------------------: | :----------------------------------------------------------: | :----------------------------------------------------------: | :-----------------------------: |
|            Name            | [Remote Database Service](https://pigsty.io/docs/pgsql/monitor/#monitor-rds) | [Existing Deployment](https://pigsty.io/docs/pgsql/monitor/#monitor-existing-cluster) |    Fully Managed Deployment     |
...
Suppose the target DB node can be managed by Pigsty (accessible via ssh and sudo is available). In that case, you can use the `pg_exporter` task in the [`pgsql.yml`](https://pigsty.io/docs/pgsql/playbook/#pgsqlyml) playbook to deploy the monitoring component PG Exporter on the target node in the same manner as a standard deployment.

You can also deploy the connection pool and its monitoring on existing instance nodes using the `pgbouncer` and `pgbouncer_exporter` tasks from the same playbook. Additionally, you can deploy host monitoring, load balancing, and log collection components using the `node_exporter`, `haproxy`, and `promtail` tasks from the [`node.yml`](https://pigsty.io/docs/node/playbook/#nodeyml) playbook, achieving a similar user experience with the native Pigsty cluster.
...
Since the target database cluster already exists, you must manually [setup monitoring users, schemas, and extensions](https://pigsty.io/docs/pgsql/monitor/#monitor-setup) on the target database cluster.

------

## Monitor RDS

If you can **only access the target database via PGURL** (database connection string), you can refer to the instructions here for configuration. In this mode, Pigsty deploys the corresponding PG Exporter on the [INFRA node](https://pigsty.io/docs/node/concept/#infra-node) to fetch metrics from the remote database, as shown below:
...
The monitoring system will no longer have host/pooler/load balancer metrics. But the PostgreSQL metrics & catalog info are still available. Pigsty has two dedicated dashboards for that: [PGRDS Cluster](https://demo.pigsty.io/d/pgrds-cluster) and [PGRDS Instance](https://demo.pigsty.io/d/pgrds-instance). Overview and Database level dashboards are reused. Since Pigsty cannot manage your RDS, you have to [setup monitor](https://pigsty.io/docs/pgsql/monitor/#monitor-setup) on the target database in advance.
...
1. Create monitoring schemas, users, and permissions on the target. Refer to [Monitor Setup](https://pigsty.io/docs/pgsql/monitor/#monitor-setup) for details.
...
![pigsty-monitor.jpg](https://pigsty.io/img/pigsty/monitor.jpg)
...
-  [Monitor User](https://pigsty.io/docs/pgsql/monitor/#monitor-user): The default username used is `dbuser_monitor`. This user belongs to the `pg_monitor` group, or ensure it has the necessary view permissions.
-  [Monitor HBA](https://pigsty.io/docs/pgsql/monitor/#monitor-hba): Default password is `DBUser.Monitor`. You need to ensure that the HBA policy allows the monitoring user to access the database from the infra nodes.
-  [Monitor Schema](https://pigsty.io/docs/pgsql/monitor/#monitor-schema): It’s optional but recommended to create a dedicate schema `monitor` for monitoring views and extensions.
-  [Monitor Extension](https://pigsty.io/docs/pgsql/monitor/#monitor-extension): It is strongly recommended to enable the built-in extension `pg_stat_statements`.
-  [Monitor View](https://pigsty.io/docs/pgsql/monitor/#monitor-view): Monitoring views are optional but can provide additional metrics. Which is recommended.
...
The monitor user here should have consistent [`pg_monitor_username`](https://pigsty.io/docs/pgsql/param/#pg_monitor_username) and [`pg_monitor_password`](https://pigsty.io/docs/pgsql/param/#pg_monitor_password) with Pigsty config inventory.
