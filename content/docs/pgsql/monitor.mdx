...
- [`pg_log_dir`](/pgsql/param#pg_log_dir) : postgres log dir, `/pg/log/postgres` by default
- [`pgbouncer_log_dir`](/pgsql/param#pgbouncer_log_dir) : pgbouncer log dir, `/pg/log/pgbouncer` by default
- [`patroni_log_dir`](/pgsql/param#patroni_log_dir) : patroni log dir, `/pg/log/patroni` by default
- [`pgbackrest_log_dir`](/pgsql/param#pgbackrest_log_dir) : pgbackrest log dir, `/pg/log/pgbackrest` by default
...
When the global flag [`patroni_ssl_enabled`](/pgsql/param#patroni_ssl_enabled) is set, the patroni target will be managed as `/etc/prometheus/targets/patroni/<ins>.yml` because it requires a different scrape endpoint (https).
...
Remote RDS targets are managed as `/etc/prometheus/targets/pgrds/<cls>.yml`. It will be created by the [`pgsql-monitor.yml`](/pgsql/playbook/#pgsql-monitoryml) playbook or `bin/pgmon-add` script.
...
|        Item \ Level        |                              L1                              |                              L2                              |               L3                |
| :------------------------: | :----------------------------------------------------------: | :----------------------------------------------------------: | :-----------------------------: |
|            Name            | [Remote Database Service](/pgsql/monitor/#monitor-rds) | [Existing Deployment](/pgsql/monitor/#monitor-existing-cluster) |    Fully Managed Deployment     |
...
Since the target database cluster already exists, you must manually [setup monitoring users, schemas, and extensions](/pgsql/monitor/#monitor-setup) on the target database cluster.

------

## Monitor RDS

If you can **only access the target database via PGURL** (database connection string), you can refer to the instructions here for configuration. In this mode, Pigsty deploys the corresponding PG Exporter on the [INFRA node](/node/arch/#infra-node) to fetch metrics from the remote database, as shown below:
...
The monitoring system will no longer have host/pooler/load balancer metrics. But the PostgreSQL metrics & catalog info are still available. Pigsty has two dedicated dashboards for that: [PGRDS Cluster](https://demo.pigsty.cc/d/pgrds-cluster) and [PGRDS Instance](https://demo.pigsty.cc/d/pgrds-instance). Overview and Database level dashboards are reused. Since Pigsty cannot manage your RDS, you have to [setup monitor](/pgsql/monitor/#monitor-setup) on the target database in advance.
...
1. Create monitoring schemas, users, and permissions on the target. Refer to [Monitor Setup](/pgsql/monitor/#monitor-setup) for details.
...
-  [Monitor User](/pgsql/monitor/#monitor-user): The default username used is `dbuser_monitor`. This user belongs to the `pg_monitor` group, or ensure it has the necessary view permissions.
-  [Monitor HBA](/pgsql/monitor/#monitor-hba): Default password is `DBUser.Monitor`. You need to ensure that the HBA policy allows the monitoring user to access the database from the infra nodes.
-  [Monitor Schema](/pgsql/monitor/#monitor-schema): It’s optional but recommended to create a dedicate schema `monitor` for monitoring views and extensions.
-  [Monitor Extension](/pgsql/monitor/#monitor-extension): It is strongly recommended to enable the built-in extension `pg_stat_statements`.
-  [Monitor View](/pgsql/monitor/#monitor-view): Monitoring views are optional but can provide additional metrics. Which is recommended.
...
The monitor user here should have consistent [`pg_monitor_username`](/pgsql/param#pg_monitor_username) and [`pg_monitor_password`](/pgsql/param#pg_monitor_password) with Pigsty config inventory.
