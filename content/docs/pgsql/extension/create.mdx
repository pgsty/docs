...
Extensions need to be [**installed**](/pgsql/extension/install) first, some extension also require [**preloading**](/pgsql/extension/config) before using.
...
|:-----------------------------:|-------------------------------|---------------------------------------------------------------------------------------------------------|
| [`pg_repack`](/e/pg_repack) | Online Bloat Control  Tools   | in the [`pg_default_extensions`](/pgsql/param#pg_default_extensions)                               |
|  [`wal2json`](/e/wal2json)  | Changing data capture in JSON | extension [without DDL](/ext/list/attr#without-ddl), [install](/pgsql/extension/install) means available |
|    [`vector`](/e/vector)    | vector data type & indexes    | in [`pg_databases`](/pgsql/param#pg_databases)  as an example                                      |
...
The [`wal2json`](/e/wal2json) is another important extension for Changing Data Capture (CDC). It is [installed](/pgsql/extension/install) by default, but it is an [**extension without DDL**](#extension-without-ddl),
