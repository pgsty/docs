...
Extensions need to be [**installed**](/pgsql/extension/install) first, some extension also requires [**preloading**](/pgsql/extension/config) before using.
...
Extensions (database logical objects) are logically part of [**PostgreSQL databases**](/pgsql/db).
In Pigsty, you can specify which extensions to be created in a database with [**`pg_databases`**](/pgsql/param#pg_databases) parameter.
...
Or install a specific version. Here's a complete example ([self-hosting supabase](/app/supabase)):
...
- Alternatively, A dictionary that may contain the following fields can be used:
  - `name`: Extension name, **REQUIRED**, beware it may differ from the [**extension package name**](/pgsql/extension/pkg).
...
If the database doesn’t exist yet, the extensions defined here will be automatically created when [**creating a cluster**](/pgsql/admin#create-cluster) or [**creating a database**](/pgsql/admin#create-database) through Pigsty.
...
But it's helpful to enlist them in the [config inventory](/config/inventory) for bookkeeping purposes. (So if you want to fork this cluster, it includes these extensions)
...
Some built-in extensions and one special [`pg_repack`](https://ext.pgsty.com/e/pg_repack) are created by default in Pigsty.

These extensions are defined by [**`pg_default_extensions`**](/pgsql/param#pg_default_extensions), created in the `template1` database and the `postgres` database by default.
...
One extra default schema `monitor` is defined by [`pg_default_schemas`](/pgsql/param#pg_default_schemas) is also created by default.
...
|:------------------------------------------------:|-------------------------------|---------------------------------------------------------------------------------------------------------------------------|
| [`pg_repack`](https://ext.pgsty.com/e/pg_repack) | Online Bloat Control  Tools   | in the [`pg_default_extensions`](/pgsql/param#pg_default_extensions)                                                      |
|  [`wal2json`](https://ext.pgsty.com/e/wal2json)  | Changing data capture in JSON | extension [without DDL](https://ext.pgsty.com/list/attr#without-ddl), [install](/pgsql/extension/install) means available |
|    [`vector`](https://ext.pgsty.com/e/vector)    | vector data type & indexes    | in [`pg_databases`](/pgsql/param#pg_databases)  as an example                                                             |

The [`pg_repack`](https://ext.pgsty.com/e/pg_repack) extension is an important utility for maintaining bloat tables online.

[**`vector`**](https://ext.pgsty.com/e/vector) is a very popular extension for RAG,
It is [**installed**](/pgsql/extension/install) by default (in the `pgsql-main` [alias](/pgsql/extension/pkg)) and created in the placeholder `meta` database in most [config template](/config/template).

The [`wal2json`](https://ext.pgsty.com/e/wal2json) is another important extension for Changing Data Capture (CDC). It is [installed](/pgsql/extension/install) by default, but it is an [**extension without DDL**](#extension-without-ddl),
...
[Extension without DDL](https://ext.pgsty.com/list/attr#without-ddl) does not require the `CREATE EXTENSION` command to work
...
| **[LOAD](/pgsql/extension/config) / DDL** | Requires `CREATE EXTENSION` | Doesn't require `CREATE EXTENSION` |
|-------------------------------------------|-----------------------------|------------------------------------|
