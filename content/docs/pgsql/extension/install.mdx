...
When installing extensions, Pigsty uses the same [**alias mapping**](https://pigsty.io/ext/usage/download/#alias-mapping) in the [**download**](https://pigsty.io/ext/usage/download/) section.
...
Install all extensions simultaneously is applicable (except two conflicts in the [`olap`](https://pigsty.io/ext/olap/) category) but not recommended. Just install the extensions you need by explicitly specifying them in the `pg_extensions` parameter.

------

## Configure

During PGSQL cluster init, Pigsty will automatically install packages (& alias) specified in [**`pg_packages`**](https://pigsty.io/docs/pgsql/param/#pg_packages) and [**`pg_extensions`**](https://pigsty.io/docs/pgsql/param/#pg_extensions).

- [**`pg_packages`**](https://pigsty.io/docs/pgsql/param/#pg_packages)
- [**`pg_extensions`**](https://pigsty.io/docs/pgsql/param/#pg_extensions)
...
By default, Pigsty also specifies 3 important extensions here: [`pgvector`](https://pigsty.io/ext/rag/vector/), [`pg_repack`](https://pigsty.io/ext/admin/pg_repack/), and [`wal2json`](https://pigsty.io/ext/etl/wal2json/) for vector search, bloat management, and CDC change extraction.
...
First, add extensions to [**`pg_extensions`**](https://pigsty.io/docs/pgsql/param/#pg_extensions), then execute the playbook subtask:
...
- **Local Repo**: You have configured using Pigsty’s local repo, and the extensions have already been [**downloaded**](https://pigsty.io/ext/usage/download/) to the local repo.
...
For production environments, we recommend using Pigsty’s local software repo to manage and install extensions uniformly: First [**download**](https://pigsty.io/ext/usage/download/) extensions to the local repo, then install them from there. This ensures consistent extension versions across your environment and prevents database nodes from directly accessing the internet. You have to do nothing when install from local repo, just make sure they are downloaded to the local repo.
...
When installing extensions, users can use [**extension aliases**](https://pigsty.io/ext/usage/intro/#package-alias) to specify extension.

The aliases will be [**translated**](https://pigsty.io/ext/usage/download/#alias-mapping) to the current active PG major version and OS environment,

and translated to the corresponding RPM/DEB package names by [**alias translation mechanism**](https://pigsty.io/ext/usage/download/#alias-mapping).

------

## Caveats

- Check [bad case](https://pigsty.io/ext/roadmap/badcase/) for known issues.
- There are two known confliction:
- [`citus`](https://pigsty.io/ext/olap/citus/) and [`hydra`](https://pigsty.io/ext/olap/hydra) are mutually exclusive, since hydra is a fork of citus columnar without renaming
- Only install one from [`pg_duckdb`](https://pigsty.io/ext/olap/pg_duckdb/), [`pg_mooncake`](https://pigsty.io/ext/olap/pg_mooncake/), [`duckdb_fdw`](https://pigsty.io/ext/olap/duckdb_fdw/), they all using their own libduckdb
