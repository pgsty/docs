...
In Pigsty, downloading and [**installing**](https://pigsty.io/ext/usage/install/) extensions are separate steps. During [**`INFRA`**](https://pigsty.io/docs/infra/) module installation, Pigsty downloads all required software to the local machine and creates a local YUM/APT repo for the entire deployment.
...
Packages defined in [**`repo_packages`**](https://pigsty.io/docs/infra/param/#repo_packages) and [**`repo_extra_packages`**](https://pigsty.io/docs/infra/param/#repo_extra_packages) are automatically downloaded to your local repo during Pigsty installation.

For PostgreSQL-related packages (core and extensions), typically put them in [**`repo_extra_packages`**](https://pigsty.io/docs/infra/param/#repo_extra_packages) while leaving `repo_packages` with its os-specific global defaults.
...
To add specific extensions, simply add Pigsty [**extension package name (`pkg`)**](https://pigsty.io/ext/usage/intro/#package-alias) to this parameter. Pigsty automatically downloads the appropriate packages for your active PG version and current OS distro.

```yaml
repo_extra_packages: [ pgsql-main, documentdb, citus, postgis, pgvector, pg_cron, rum ]
```

To download **all available** extensions for the current PG version, add all **16** extension category aliases (as in the [`rich`](https://pigsty.io/docs/conf/rich/) config template):
...
When using these aliases, the `$v` placeholder is replaced with the PostgreSQL major version number from [`pg_version`](https://pigsty.io/docs/pgsql/param/#pg_version) (default: 17).

To download packages for different PostgreSQL versions, either:

- Change the [**`pg_version`**](https://pigsty.io/docs/pgsql/param/#pg_version) parameter, or
...
- Have extensive dependencies (like [**`pl/R`**](https://pigsty.io/ext/lang/plr/))
- Depend on commercial software (like [**`oracle_fdw`**](https://pigsty.io/ext/fdw/oracle_fdw/))
