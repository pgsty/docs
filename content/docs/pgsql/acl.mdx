...
- Offline (`dbrole_offline`): Role for restricted read-only access ([offline](/docs/pgsql/config/#offline) instance)

Default roles are defined in [`pg_default_roles`](/docs/pgsql/param#pg_default_roles), change default roles is not recommended.
...
To define extra options, specify them in [`pg_default_roles`](/docs/pgsql/param#pg_default_roles):
...
- Database connect privilege is covered by database [definition](/docs/pgsql/db/#define-database)
...
Default object privileges are defined in [`pg_default_privileges`](/docs/pgsql/param#pg_default_privileges).
...
These SQL commands will be executed on `postgres` & `template1` during cluster bootstrap, and newly created databases will inherit it from `tempalte1` by default.
...
Database privilege is covered by [database definition](/docs/pgsql/db/#define-database).
...
- If `owner` exists, it will be used as the database owner instead of default [`{{ pg_dbsu }}`](/docs/pgsql/param#pg_dbsu)
