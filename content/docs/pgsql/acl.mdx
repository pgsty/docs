---
title: Privileges
description: Access Control with default roles and privileges
icon: Fingerprint
---

## acl

Pigsty has a default role system consist of four [default roles](https://pigsty.io/docs/pgsql/acl/#default-roles) and four [default users](https://pigsty.io/docs/pgsql/acl/#default-users):
...
- Offline (`dbrole_offline`): Role for restricted read-only access ([offline](https://pigsty.io/docs/pgsql/config/#offline) instance)

Default roles are defined in [`pg_default_roles`](https://pigsty.io/docs/pgsql/param/#pg_default_roles), change default roles is not recommended.
...
- [`pg_dbsu`](https://pigsty.io/docs/pgsql/param/#pg_dbsu) : os dbsu name, postgres by default, better not change it
- [`pg_replication_username`](https://pigsty.io/docs/pgsql/param/#pg_replication_username) : postgres replication username, `replicator` by default
- [`pg_replication_password`](https://pigsty.io/docs/pgsql/param/#pg_replication_password) : postgres replication password, `DBUser.Replicator` by default
- [`pg_admin_username`](https://pigsty.io/docs/pgsql/param/#pg_admin_username) : postgres admin username, `dbuser_dba` by default
- [`pg_admin_password`](https://pigsty.io/docs/pgsql/param/#pg_admin_password) : postgres admin password in plain text, `DBUser.DBA` by default
- [`pg_monitor_username`](https://pigsty.io/docs/pgsql/param/#pg_monitor_username) : postgres monitor username, `dbuser_monitor` by default
- [`pg_monitor_password`](https://pigsty.io/docs/pgsql/param/#pg_monitor_password) : postgres monitor password, `DBUser.Monitor` by default
...
To define extra options, specify them in [`pg_default_roles`](https://pigsty.io/docs/pgsql/param/#pg_default_roles):
...
Pigsty has a battery-included privilege model that works with [default roles](https://pigsty.io/docs/pgsql/acl/#default-roles).
...
- Database connect privilege is covered by database [definition](https://pigsty.io/docs/pgsql/db/#define-database)
...
Default object privileges are defined in [`pg_default_privileges`](https://pigsty.io/docs/pgsql/param/#pg_default_privileges).
...
[`ALTER DEFAULT PRIVILEGES`](https://www.postgresql.org/docs/current/sql-alterdefaultprivileges.html) allows you to set the privileges that will be applied to objects created in the future. It does not affect privileges assigned to already-existing objects, and objects created by non-admin users.
...
Which will be rendered in [`pg-init-template.sql`](https://github.com/pgsty/pigsty/blob/main/roles/pgsql/templates/pg-init-template.sql) alone with `ALTER DEFAULT PRIVILEGES` statement for admin users.
...
1. [`{{ pg_dbsu }}`](https://pigsty.io/docs/pgsql/param/#pg_dbsu), `postgres` by default
2. [`{{ pg_admin_username }}`](https://pigsty.io/docs/pgsql/param/#pg_admin_username), `dbuser_dba` by default
...
Database privilege is covered by [database definition](https://pigsty.io/docs/pgsql/db/#define-database).
...
- If `owner` exists, it will be used as database owner instead of default [`{{ pg_dbsu }}`](https://pigsty.io/docs/pgsql/param/#pg_dbsu)
- If `revokeconn` is `false`, all users have the `CONNECT` privilege of the database, this is the default behavior.
- If `recokeconn` is set to `true` explicitly:
