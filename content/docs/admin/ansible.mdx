...
Pigsty implements admin controllers with [Ansible](https://docs.ansible.com/ansible/).
Which is an open source automation tool for managing large-scale infrastructure in an Infra-as-Code (IaC) manner.
Wildly used in the industry by operators.
...
Ansible is available on macOS too, You can install macOS on your Macbook with [**Homebrew**](https://brew.sh/).
...
Running the `./node.yml` playbook essentially turns to `ansible-playbook node.yml`.
