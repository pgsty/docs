...
Pigsty [**install**](/docs/install) from Internet upstream by default, while some environments are isolated from the Internet.
...
it is essentially a snapshot tarball of local APT / YUM repo, taken after a normal [installation](/docs/install).
...
We usually release offline packages for the following [linux distro](/docs/prepare/linux), with the latest OS minor version.
...
<Steps>
    ...
    <Step>
        Run standard [online installation procedure](/docs/install)
    </Step>
