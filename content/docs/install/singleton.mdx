...
[Prepare](#prepare) an [ssh-accessible](/docs/prepare#accessibility) [node](/docs/prepare#node) with [Compatible Linux OS](/docs/prepare/linux),
...
<Steps>
    ...
    <Step>
        [**Configure**](#configure) the [`pigsty.yml`](/docs/config/inventory) inventory file according
    ...
    <Step>
        ...
        [**Install**](#install) everything accoring to your config:
...
|     [SSH](/docs/prepare/admin#ssh)     | `nopass` via public key             | [Accessible](/docs/prepare/admin#check-accessibility) | `ssh <ip\|alias> sudo ls` witout error               |
...
You can also install via `git`, `pig`, or [**download source**](/docs/prepare/software#pigsty) & [offline package](/docs/install/offline#what-isoffline-package) tarball directly from [GitHub](https://github.com/pgsty/pigsty).
...
The [`bootstrap`](/docs/install/offline#bootstrap) script will try its best to install [ansible](/docs/admin/ansible) and its dependencies (`jmespath`) for you

```bash title="~/pigsty"
./bootstrap
```

In case of no Internet access, get [offline package](/docs/install/offline#what-is-offline-package) to `/tmp/pkg.tgz`, the script will [offline install](/docs/install/offline#bootstrap) from it.
...
There are many [**config tempalte**](/docs/config/template) for your reference, But here are some quick examples:
...
Beware of this when you are configure pigsty [manully](/docs/config/tutorial). Check the generated `pigsty.yml` to proceed.

### HEY! Don't forget these passwords! [!toc]

<Callout title="Change default passwords!" type="warn">

PLEASE <a href="/docs/config/security#passwords"><span className="text-red-500 font-bold">CHANGE DEFAULT PASSWORDS</span></a> in any serious deployment before [install](#install)

</Callout>

Then change default [passwords](/docs/config/security#passwords) and make necessary adjustments, the final `pigsty.yml` may looks like:
