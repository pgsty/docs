...
[准备](#prepare) 一台具有 [SSH 访问权限](/zh/prepare/admin#check-accessibility) 的 [节点](/zh/prepare/hardware#node) 并安装 [兼容的 Linux 发行版](/zh/prepare/linux)，
使用免密 [`ssh`](/zh/prepare/admin#ssh) 和 [`sudo`](/zh/prepare/admin#sudo-privileges) 权限的用户运行：

<Steps>
    <Step>

        使用以下命令 [**下载**](#download) pigsty：
    ...
    <Step>
        根据您的需求和环境 [**配置**](#configure) [`pigsty.yml`](/zh/config/inventory) 配置清单文件。
...
|               项目                | 要求                           |                       项目                       | 要求                                          |
|:---------------------------------:|:--------------------------------------|:------------------------------------------------:|:-----------------------------------------------------|
|  [节点](/zh/prepare/hardware#node)   | 至少 `1C1G`，推荐 `2C2G`   |          [规格](/zh/prepare/hardware#spec)          | 至少 `1` 个节点，`2` 个用于半高可用，`3+` 个用于真正高可用 |
...
| [语言环境](/zh/prepare/software#linux) | `C.UTF-8` 或 `C`                      |      [防火墙](/zh/prepare/software#firewall)      | 端口：`80` / `443` / `22` / `5432`                   |
|    [用户](/zh/prepare/admin#user)    | 避免使用 `root` 和 `postgres`       |      [Sudo](/zh/prepare/admin#sudo-privileges)      | `nopass` sudo 权限                              |
|     [SSH](/zh/prepare/admin#ssh)     | 通过公钥 `nopass`               | [可访问性](/zh/prepare/admin#check-accessibility) | `ssh <ip\|alias> sudo ls` 无错误              |
