...
- [Create Cluster](/docs/etcd/admin/#create-cluster)
- [Remove Cluster](/docs/etcd/admin/#remove-cluster)
- [CLI Environment](/docs/etcd/admin/#cli-environment)
- [Reload Config](/docs/etcd/admin/#reload-config)
- [Add Member](/docs/etcd/admin/#add-member)
- [Remove Member](/docs/etcd/admin/#remove-member)
...
Pigsty has safeguard mechanism to prevent accidental purge. By default, [`etcd_clean`](/docs/etcd/param#etcd_clean) is `true`, and [`etcd_safeguard`](/docs/etcd/param#etcd_safeguard) is `false`, which means the playbook will purge etcd cluster even if there are running etcd instances. In this case, `etcd.yml` is truly idempotent, which is useful for development, testing, and emergency rebuild of etcd cluster in production.
...
To remove an existing etcd cluster, use the `etcd_clean` subtask of [`etcd.yml`](/docs/etcd/playbook/#etcdyml), do think before you type.
...
The `etcd_clean` subtask will honor the [`etcd_safeguard`](/docs/etcd/param#etcd_safeguard), while the `etcd_purge` subtask will ignore that and wipe out the entire etcd cluster.
...
5. update etcd endpoints reference with [reload-config](/docs/etcd/admin/#reload-config)
...
1. remove/uncomment it from inventory and [reload config](/docs/etcd/admin/#reload-config)
