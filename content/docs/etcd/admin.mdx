...
- [Create Cluster](https://pigsty.io/docs/etcd/admin/#create-cluster)
- [Remove Cluster](https://pigsty.io/docs/etcd/admin/#remove-cluster)
- [CLI Environment](https://pigsty.io/docs/etcd/admin/#cli-environment)
- [Reload Config](https://pigsty.io/docs/etcd/admin/#reload-config)
- [Add Member](https://pigsty.io/docs/etcd/admin/#add-member)
- [Remove Member](https://pigsty.io/docs/etcd/admin/#remove-member)

Check [ETCD: FAQ](https://pigsty.io/docs/etcd/faq/) for more questions.

------

## Create Cluster

To create an etcd cluster, define the `etcd` cluster in [inventory](https://pigsty.io/docs/setup/config/#inventory) first:
...
Then run the [`etcd.yml`](https://pigsty.io/docs/etcd/playbook/#etcdyml) playbook.

```bash
./etcd.yml   # init etcd module on group 'etcd'
```

Pigsty has safeguard mechanism to prevent accidental purge. By default, [`etcd_clean`](https://pigsty.io/docs/etcd/param/#etcd_clean) is `true`, and [`etcd_safeguard`](https://pigsty.io/docs/etcd/param/#etcd_safeguard) is `false`, which means the playbook will purge etcd cluster even if there are running etcd instances. In this case, `etcd.yml` is truly idempotent, which is useful for development, testing, and emergency rebuild of etcd cluster in production.
...
To remove an existing etcd cluster, use the `etcd_clean` subtask of [`etcd.yml`](https://pigsty.io/docs/etcd/playbook/#etcdyml), do think before you type.
...
The `etcd_clean` subtask will honor the [`etcd_safeguard`](https://pigsty.io/docs/etcd/param/#etcd_safeguard), while the `etcd_purge` subtask will ignore that and wipe out the entire etcd cluster.
...
5. update etcd endpoints reference with [reload-config](https://pigsty.io/docs/etcd/admin/#reload-config)
...
1. remove/uncomment it from inventory and [reload config](https://pigsty.io/docs/etcd/admin/#reload-config)
