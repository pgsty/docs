...
- Setup node name
- Setup node repo and install packages
- Sync clock time via `chrony`
- System tuning via `tuned`
- Service exposure via `haproxy`
- Host monitoring via `node_exporter`
- Log collection via `promtail`
- L2 VIP support via `keepalived` (**OPTIONAL**)
