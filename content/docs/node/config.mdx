...
You don't need to define node cluster and node instances on purpose.
...
| [`node_cluster`](/docs/node/#node_cluster) | `cls` | **C** | `{{ pg_cluster }}`              | **PG Cluster Name**  |
...
If there's no corresponding PG instance defined, and no explicit [`nodename`](/docs/node/param#nodename) & [`node_cluster`](/docs/node/#node_cluster) defined,
...
If your node cannot be simply access via `ssh <inventory_hostname>`, [ssh alias](/docs/prepare/alias) and more [ansible conn param](https://docs.ansible.com/ansible/latest/inventory_guide/intro_inventory.html#connecting-to-hosts-behavioral-inventory-parameters) (even a different IP) can be used.
