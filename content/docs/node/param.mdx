...
Each node has **identity parameters** that are configured through the parameters in `<cluster>.hosts` and `<cluster>.vars`. Check [NODE Identity](/docs/node/#configuration) for details.
...
It is used by [`PGSQL`](/docs/pgsql/) [Service](https://pigsty.io/docs/concept/svc/).
