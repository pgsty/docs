---
title: Parameter
description:  64 parameters to customize node
full: true
---
...
Each node has **identity parameters** that are configured through the parameters in `<cluster>.hosts` and `<cluster>.vars`. Check [NODE Identity](https://pigsty.io/docs/node/#configuration) for details.
...
When this parameter is enabled, node names are exchanged between the same group of nodes executing the [`node.yml`](https://pigsty.io/docs/node/playbook/#nodeyml) playbook, written to `/etc/hosts`.
...
It should slightly larger than [`pg_shared_buffer_ratio`](https://pigsty.io/docs/pgsql/param/#pg_shared_buffer_ratio), if not zero.
...
It should be equal or slightly larger than [`pg_shared_buffer_ratio`](https://pigsty.io/docs/pgsql/param/#pg_shared_buffer_ratio), if not zero.
...
Usually, the database tuning template [`pg_conf`](https://pigsty.io/docs/pgsql/param/#pg_conf) should be paired with the node tuning template: [`node_tune`](#node_tune)
...
It is used by [`PGSQL`](https://pigsty.io/docs/pgsql/) [Service](https://pigsty.io/docs/concept/svc/).
...
> Log directory are customizable according to [`pg_log_dir`](https://pigsty.io/docs/pgsql/param/#pg_log_dir), [`patroni_log_dir`](https://pigsty.io/docs/pgsql/param/#patroni_log_dir), [`pgbouncer_log_dir`](https://pigsty.io/docs/pgsql/param/#pgbouncer_log_dir), [`pgbackrest_log_dir`](https://pigsty.io/docs/pgsql/param/#pgbackrest_log_dir)
