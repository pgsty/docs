...
Here is a tutorial to help you to build a complex config file [inventory](/docs/config/inventory) from scratch.
...
To provide PostgreSQL service, you have to define additional groups and install [**PGSQL**](/docs/pgsql) & [**ETCD**](/docs/etcd) modules.
...
- `pg_databases`: define a new database `meta` with pigsty [CMDB](/docs/config/cmdb) schema and `vector` extension
...
Check the [App: Pgadmin](/docs/app/pgadmin) for details on how to run pgAdmin with Pigsty. Short version:
...
Check the [App: Supabase](/docs/app/supabase) for details on how to run Supabase with Pigsty. Short version:
