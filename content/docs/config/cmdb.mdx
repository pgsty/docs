...
You can use PostgreSQL as [config inventory](/docs/config/inventory) for Pigsty, replacing the static YAML configuration file.

With Postgres CMDB, configuration is organized in a structured relational tables, which can be easily queried and manipulated using SQL.
...
CMDB schema is loaded by default, there are 3 util scripts to help you to switch to the CMDB:
...
Rrun `bin/inventory_load` without arguments will load the default `pigsty.yml` into the default CMDB.
