...
It's **OPTIONAL**, you can edit the `pigsty.yml` directly as [**tutorial**](/docs/config/tutorial) show.
...
<Callout title="Region">
    If `-r|--region` is specified, it will use the specified region directly, `china` mirror will be used if google is banned.
</Callout>
...
<Callout title="Locale Setting">
    ...
    - **OR**, Your system support `C.utf8` / `C.utf-8` locale (`local -a`)
