...
If the download speed of some packages is too slow, you can set the download proxy to complete the first download by using the [`proxy_env`](#proxy_env) config entry or directly download the pre-packaged [offline package](https://pigsty.io/docs/install/offline/), which is essentially a local software source built on the same operating system.
...
- `yum`: Install using yum (if yum installation is enabled, run yum to install [`node_exporter`](/docs/node/param/#node_exporter) and [`pg_exporter`](/docs/pgsql/param#pg_exporter) before deploying Exporter)
- `binary`: Install using a copy binary (copy [`node_exporter`](/docs/node/param/#node_exporter) and [`pg_exporter`](/docs/pgsql/param#pg_exporter) binary directly from the meta node, not recommended)
