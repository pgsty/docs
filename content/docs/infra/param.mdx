...
The default value `10.10.10.10` is a placeholder which will be replaced during [configure](/docs/config/configure)
...
- [`node_repo_modules`](/docs/node/param#node_repo_modules)
...
- endpoint: Usually used as an alternative to path, specifies the upstream server address. Setting endpoint indicates this is a reverse proxy server
    - In the configuration, `${admin_ip}` can be used as a placeholder, and will be dynamically replaced with [admin_ip](/docs/infra/param#admin_ip) during deployment
...
If the download speed of some packages is too slow, you can set the download proxy to complete the first download by using the [`proxy_env`](#proxy_env) config entry or directly download the pre-packaged [offline package](/docs/install/offline), which is essentially a local software source built on the same operating system.
...
You have to add `nameserver {{ admin_ip }}` to your `/etc/resolv` to use this dns server, and [`node_dns_servers`](/docs/node/param#node_dns_servers) will do the trick.
...
If you don’t want to use the default DNS server, you can set this value to `false` to disable it. And use [`node_default_etc_hosts`](/docs/node/param#node_default_etc_hosts) and [`node_etc_hosts`](/docs/node/param#node_etc_hosts) instead.
...
- `none`: No installation, (by default, the Exporter has been previously installed by the [`node_pkg`](/docs/node/param#node_default_packages) task)
- `yum`: Install using yum (if yum installation is enabled, run yum to install [`node_exporter`](/docs/node/param#node_exporter) and [`pg_exporter`](/docs/pgsql/param#pg_exporter_enabled) before deploying Exporter)
- `binary`: Install using a copy binary (copy [`node_exporter`](/docs/node/param#node_exporter) and [`pg_exporter`](/docs/pgsql/param#pg_exporter_enabled) binary directly from local file dir, not recommended)
