...
The default value `10.10.10.10` is a placeholder that will be replaced during [configure](/docs/config/configure)
...
If the download speed of some packages is too slow, you can set the download proxy to complete the initial download by using the [`proxy_env`](#proxy_env) configuration entry or directly download the pre-packaged [offline package](/docs/install/offline), which is essentially a local software source built on the same operating system.
