...
| [`alertmanager_port`](#alertmanager_port)                   | [`PROMETHEUS`](#prometheus)       | `arg         | ` G   | alertmanager listen port, `9093` by default        |
...
The default value `10.10.10.10` is a placeholder which will be replaced during [configure](https://pigsty.io/docs/setup/install/#configure)
...
- [`node_default_etc_hosts`](https://pigsty.io/docs/node/param/#node_default_etc_hosts)
- [`node_etc_hosts`](https://pigsty.io/docs/node/param/#node_etc_hosts)
- [`node_repo_modules`](https://pigsty.io/docs/node/param/#node_repo_modules)
...
- endpoint: Usually used as an alternative to path, specifies the upstream server address. Setting endpoint indicates this is a reverse proxy server
    - In the configuration, `${admin_ip}` can be used as a placeholder, and will be dynamically replaced with [admin_ip](https://pigsty.io/docs/infra/param/#admin_ip) during deployment
...
If the download speed of some packages is too slow, you can set the download proxy to complete the first download by using the [`proxy_env`](#proxy_env) config entry or directly download the pre-packaged [offline package](https://pigsty.io/docs/setup/offline/), which is essentially a local software source built on the same operating system.
...
When leaving it empty, Pigsty will use the default values from the `repo_upstream_default` defined in [`roles/node_id/vars`](https://github.com/pgsty/pigsty/blob/main/roles/node_id/vars/) according to you OS.
...
When leaving it empty, Pigsty will use the default values from the `repo_packages_default` defined in [`roles/node_id/vars`](https://github.com/pgsty/pigsty/blob/main/roles/node_id/vars/) according to you OS.
...
This parameter is same as [`repo_packages`](#repo_packages), but it is used for the additional software packages that need to be downloaded. (Usually PG version ad hoc packages)
...
Here `$v` will be replaced with the actual PostgreSQL major version number [`pg_version`](https://pigsty.io/docs/pgsql/param/#pg_version), So you can add any PG version related packages here, and Pigsty will download them for you.
...
Beware that if the [`region`](#region) is set to `china`, the `pigsty.io` will be replaced with `pigsty.cc` automatically.
...
by default
  ...
  - also redirect 80 port to 443 port for all non-default servers in nginx [`infra_portal`](#infra_portal)
...
You have to add `nameserver {{ admin_ip }}` to your `/etc/resolv` to use this dns server, and [`node_dns_servers`](https://pigsty.io/docs/node/param/#node_dns_servers) will do the trick.
...
If you don’t want to use the default DNS server, you can set this value to `false` to disable it. And use [`node_default_etc_hosts`](https://pigsty.io/docs/node/param/#node_default_etc_hosts) and [`node_etc_hosts`](https://pigsty.io/docs/node/param/#node_etc_hosts) instead.
...
- `none`: No installation, (by default, the Exporter has been previously installed by the [`node_pkg`](https://pigsty.io/docs/node/param/#node_default_packages) task)
- `yum`: Install using yum (if yum installation is enabled, run yum to install [`node_exporter`](https://pigsty.io/docs/node/param/#node_exporter) and [`pg_exporter`](https://pigsty.io/docs/pgsql/param/#pg_exporter) before deploying Exporter)
- `binary`: Install using a copy binary (copy [`node_exporter`](https://pigsty.io/docs/node/param/#node_exporter) and [`pg_exporter`](https://pigsty.io/docs/pgsql/param/#pg_exporter) binary directly from the meta node, not recommended)
