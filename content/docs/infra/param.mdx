...
Here `$v` will be replaced with the actual PostgreSQL major version number [`pg_version`](/docs/pgsql/param#pg_version), So you can add any PG version related packages here, and Pigsty will download them for you.
...
- `yum`: Install using yum (if yum installation is enabled, run yum to install [`node_exporter`](https://pigsty.io/docs/node/param/#node_exporter) and [`pg_exporter`](/docs/pgsql/param#pg_exporter) before deploying Exporter)
- `binary`: Install using a copy binary (copy [`node_exporter`](https://pigsty.io/docs/node/param/#node_exporter) and [`pg_exporter`](/docs/pgsql/param#pg_exporter) binary directly from the meta node, not recommended)
