...
The `10.10.10.10` IP placeholder in the `infra` group will be replaced with the [**current node’s primary IP address**](/docs/prepare/#network-configuration) during [**configuration**](/docs/config/configure), meaning the INFRA module will be installed on the current node.
