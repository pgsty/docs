...
To improve resource utilization, PostgreSQL high availability typically relies on the [**`ETCD`**](/docs/etcd/) module, which can share nodes with the INFRA module.
...
To install the INFRA module on nodes, first add node IPs to the `infra` group in the [**inventory**](/docs/config/inventory) and assign them an Infra instance number [`infra_seq`](/docs/infra/param/#infra_seq).
...
The `10.10.10.10` IP placeholder in the `infra` group will be replaced with the [**current node’s primary IP address**](https://pigsty.io/docs/prepare/#network-configuration) during [**configuration**](/docs/config/configure), meaning the INFRA module will be installed on the current node.

Then use the [`infra.yml`](/docs/infra/playbook/#infrayml) playbook to initialize the INFRA module on the node.
...
In addition to configuring VIP-related parameters like [`vip_address`](/docs/node/param/#vip_address), you need to modify endpoints for Infra services in [`infra_portal`](/docs/infra/param/#infra_portal).