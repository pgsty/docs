...
To improve resource utilization, PostgreSQL high availability typically relies on the [**`ETCD`**](https://pigsty.io/docs/etcd/) module, which can share nodes with the INFRA module.
...
To install the INFRA module on nodes, first add node IPs to the `infra` group in the [**inventory**](https://pigsty.io/docs/setup/config/) and assign them an Infra instance number [`infra_seq`](https://pigsty.io/docs/infra/param/#infra_seq).
...
The `10.10.10.10` IP placeholder in the `infra` group will be replaced with the [**current node’s primary IP address**](https://pigsty.io/docs/setup/prepare/#network-configuration) during [**configuration**](https://pigsty.io/docs/setup/install/#configure), meaning the INFRA module will be installed on the current node.

Then use the [`infra.yml`](https://pigsty.io/docs/infra/playbook/#infrayml) playbook to initialize the INFRA module on the node.
...
In addition to configuring VIP-related parameters like [`vip_address`](https://pigsty.io/docs/node/param/#vip_address), you need to modify endpoints for Infra services in [`infra_portal`](https://pigsty.io/docs/infra/param/#infra_portal).