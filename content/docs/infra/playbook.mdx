...
- [`infra.yml`](/docs/infra/playbook/#infrayml): Initialize Pigsty infrastructure on infra nodes
- [`infra-rm.yml`](/docs/infra/playbook/#infra-rmyml): Remove infrastructure components from infra nodes
- [`install.yml`](/docs/infra/playbook/#installyml): Perform a complete one-time installation of Pigsty on the current node

------

## `infra.yml`

The INFRA module playbook [`infra.yml`](https://github.com/pgsty/pigsty/blob/main/infra.yml) initializes the infrastructure module on [**Infra nodes**](/docs/node/concept/#infra-nodes) defined in the `infra` group of your config file.
...
- Incorporates the current Infra node as a [regular node](/docs/node/concept/#regular-nodes) managed by Pigsty
- Deploys **infrastructure** components, including Prometheus, Grafana, Loki, Alertmanager, PushGateway, Blackbox Exporter, etc.

**This playbook executes on the [`infra`](/docs/node/concept/#infra-nodes) group by default:**

- Pigsty installs the [**`INFRA`**](/docs/infra/) module on the group named `infra` in the config file
- During [configure](/docs/config/configure), Pigsty marks the current installation node as an Infra node and replaces the placeholder IP address `10.10.10.10` in the config template with the [**current node’s primary IP address**](https://pigsty.io/docs/prepare/#network-preparation)
...
This playbook is described in more detail in [Playbook: One-time Installation](/docs/admin/playbook#deployment).