...
- [`infra.yml`](https://pigsty.io/docs/infra/playbook/#infrayml): Initialize Pigsty infrastructure on infra nodes
- [`infra-rm.yml`](https://pigsty.io/docs/infra/playbook/#infra-rmyml): Remove infrastructure components from infra nodes
- [`install.yml`](https://pigsty.io/docs/infra/playbook/#installyml): Perform a complete one-time installation of Pigsty on the current node

------

## `infra.yml`

The INFRA module playbook [`infra.yml`](https://github.com/pgsty/pigsty/blob/main/infra.yml) initializes the infrastructure module on [**Infra nodes**](https://pigsty.io/docs/node/concept/#infra-nodes) defined in the `infra` group of your config file.
...
- Incorporates the current Infra node as a [regular node](https://pigsty.io/docs/node/concept/#regular-nodes) managed by Pigsty
- Deploys **infrastructure** components, including Prometheus, Grafana, Loki, Alertmanager, PushGateway, Blackbox Exporter, etc.

**This playbook executes on the [`infra`](https://pigsty.io/docs/node/concept/#infra-nodes) group by default:**

- Pigsty installs the [**`INFRA`**](https://pigsty.io/docs/infra/) module on the group named `infra` in the config file
- During [configure](https://pigsty.io/docs/setup/install/#configure), Pigsty marks the current installation node as an Infra node and replaces the placeholder IP address `10.10.10.10` in the config template with the [**current node’s primary IP address**](https://pigsty.io/docs/setup/prepare/#network-preparation)
...
- This is an idempotent playbook - repeated execution will
...
infrastructure components on Infra nodes

- Unless [`prometheus_clean`](https://pigsty.io/docs/infra/param/#prometheus_clean) is set to `false`, Prometheus time series metrics will be lost
- Unless [`loki_clean`](https://pigsty.io/docs/infra/param/#loki_clean) is set to `false`, Loki log data will be lost
- Unless [`grafana_clean`](https://pigsty.io/docs/infra/param/#grafana_clean) is set to `false`, Grafana dashboards and configuration changes will be lost

- When the local software repository
...
exists, this playbook skips downloading software from the internet
...
The INFRA module playbook [`infra-rm.yml`](https://github.com/pgsty/pigsty/blob/main/infra-rm.yml) removes Pigsty infrastructure from [Infra nodes](https://pigsty.io/docs/node/#infra-nodes) defined in the `infra` group of your config file.
...
This playbook is described in more detail in [Playbook: One-time Installation](https://pigsty.io/docs/setup/playbook/#one-time-installation).