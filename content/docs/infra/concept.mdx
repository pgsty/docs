...
The INFRA module isn’t mandatory for HA PostgreSQL - for instance, it’s omitted in [**Slim Install**](https://pigsty.io/docs/install/slim/) mode.
...
The [PGSQL](/docs/pgsql/) module relies on several [INFRA](/docs/node/#infra节点) services:
...
Pigsty’s [offline package](https://pigsty.io/docs/install/offline/) is a tarball of a pre-built repo directory. If `/www/pigsty` exists with a `/www/pigsty/repo_complete` marker, Pigsty skips downloading from upstream - perfect for air-gapped environments!
