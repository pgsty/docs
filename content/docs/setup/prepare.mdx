...
Pigsty requires [**Ansible**](/docs/setup/playbook#ansible) on the [**admin node**](/docs/node/concept#admin-node) for deployment orchestration.

<Callout title="Ansible requirement" type="info">

    Only the admin node requires Ansible. Regular nodes do not need it installed.

</Callout>

### Install Ansible

The [**bootstrap**](/docs/setup/offline#bootstrap) procedure handles Ansible installation automatically. 
For manual installation, use the appropriate command for your OS:

```bash
# EL 8/9 (RHEL, Rocky, Alma, etc.)
...
Or download a specific version from [GitHub Releases](https://github.com/pgsty/pigsty/releases/).

<Callout title="Offline installation" type="info">

    For environments without Internet access, use [offline packages](/docs/setup/offline#offline-packages) pre-packed for different OS distributions.
