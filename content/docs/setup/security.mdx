...
Pigsty already provides a secure-by-default [authentication](https://pigsty.io/docs/pgsql/hba/) and [access control](/docs/pgsql/acl) model, which is sufficient for most scenarios.

[![pigsty-acl.jpg](https://pigsty.io/img/pigsty/acl.jpg)](/docs/pgsql/acl)
...
- Please change pgbackrest references: [`pgbackrest_repo`.`minio`.`s3_key_secret`](/docs/pgsql/param#pgbackrest_repo)

**If you are using remote backup method, secure backup with distinct passwords**

- Use `aes-256-cbc` for [`pgbackrest_repo`.`*`.`cipher_type`](/docs/pgsql/param#pgbackrest_repo)
