...
You can run the latest PostgreSQL 17(,16,15) kernels, (supabase is using the 15 currently), alone with [404](/ext/list/) PostgreSQL extensions out-of-the-box. Run on [mainstream](/docs/prepare/linux/) Linus OS distros
with production grade [HA](/docs/concept/ha/) [PostgreSQL](/docs/pgsql/), [MinIO](/docs/minio/), Prometheus & Grafana Stack for observability, and Nginx for reverse proxy.

Since most of the supabase maintained extensions are not available in the official PGDG repo, we have compiled all the RPM/DEBs for these extensions and put them in the [Pigsty repo](/ext/list/):

- [pg_graphql](/ext/feat/pg_graphql/): GraphQL support in PostgreSQL (Rust extension via PIGSTY)
- [pg_jsonschema](/ext/feat/pg_jsonschema/): JSON Schema validation (Rust extension via PIGSTY)
- [wrappers](/ext/fdw/wrappers/): Supabase’s foreign data source wrappers bundle (Rust extension via PIGSTY)
- [index_advisor](/ext/feat/index_advisor/): Query index advisor (SQL extension via PIGSTY)
- [pg_net](/ext/util/pg_net/): Async non-blocking HTTP/HTTPS requests in SQL (C extension via PIGSTY)
- [vault](/ext/sec/supabase_vault/): Store encrypted credentials in Vault (C extension via PIGSTY)
- [pgjwt](/ext/util/pgjwt/): PostgreSQL implementation of JSON Web Token API (SQL extension via PIGSTY)
- [pgsodium](/ext/sec/pgsodium/): Table data encryption storage TDE (C extension via PIGSTY)
- [supautils](/ext/sec/supautils/): Secure database clusters in cloud environments (C extension via PIGSTY)
- [pg_plan_filter](/ext/feat/plan_filter/): Block specific queries using execution plan cost filtering (C extension via PIGSTY)
...
Pigsty’s supabase is based on the [Supabase Docker Compose Template](https://supabase.com/docs/guides/self-hosting/docker), with some slight modifications to fit-in Pigsty’s default [ACL](/docs/pgsql/acl/) model.
...
The built-in [`supa.yml`](https://github.com/pgsty/pigsty/blob/main/conf/supa.yml) [config](/docs/config/supa) template will create a single-node supabase, with a [singleton PostgreSQL](/docs/pgsql/) and SNSD [MinIO](/docs/minio/) server. You can use [Multinode PostgreSQL Clusters](/docs/pgsql/config/#replica) and [MNMD MinIO Clusters](/docs/minio/config/#multi-node-multi-drive) / external S3 service instead in production, we will cover that later.
...
-  [**Hardware**](/docs/setup/prepare/#node): necessary VM/BM resources, one node at least, 3-4 are recommended for HA.
-  [**Linux OS**](/docs/setup/prepare/#operating-system): Linux x86_64 server with fresh installed Linux, [check compatible distro](/docs/reference/compatibility/)
-  [**Network**](/docs/setup/prepare/#network): Static IPv4 address which can be used as node identity
-  [**Admin User**](/docs/setup/prepare/#admin-user): nopass ssh & sudo are recommended for admin user
-  [**Conf Template**](/docs/setup/config/): Use the [`supa`](/docs/config/template) config template, if you don’t know how to manually configure pigsty
...
- [Security Enhancement](/docs/app/supabase/#security-enhancement)
- [Domain Name and HTTPS](/docs/app/supabase/#domain-name-and-https)
- [Sending Mail with SMTP](/docs/app/supabase/#sending-mail-with-smtp)
- [MinIO or External S3](/docs/app/supabase/#minio-or-external-s3)
- [True High Availability](/docs/app/supabase/#true-high-availability)
...
- [`postgres_password`](https://github.com/pgsty/pigsty/blob/main/conf/supa.yml#L126), according to [`pg_users`](/docs/pgsql/user/)
...
You can [refer to the certbot tutorial](/docs/tasks/cerbot) to apply for a free HTTPS certificate. Here we assume your custom domain name is: `supa.pigsty.cc`, then you should modify the `supa` domain in [`infra_portal`](/docs/infra/param/#infra_portal) like this:
...
Pigsty’s self-hosting supabase will use a local [SNSD MinIO](/docs/minio/config/#single-node-single-drive) server, which is used by Supabase itself for object storage, and by PostgreSQL for backups. For production use, you should consider using a HA [MNMD MinIO](/docs/minio/config/#multi-node-multi-drive) cluster or an external S3 compatible service instead.
...
In the supabase template, Pigsty has already used a daily full backup at 01:00 AM, you can refer to the [backup/restore](/docs/pgsql/pitr/) to modify the backup strategy.
...
More about backup strategy, please refer to [**Backup Strategy**](/docs/pgsql/pitr/)
...
We recommend you to refer to the [trio](/docs/config/template) and [safe](/docs/config/template) config to upgrade your cluster to three nodes or more.
