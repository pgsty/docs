...
You can run the latest PostgreSQL 17(,16,15) kernels, (supabase is using the 15 currently), alone with [423](/ext/list/) PostgreSQL extensions out-of-the-box. Run on [mainstream](/docs/prepare/linux/) Linus OS distros
...
- [pg_graphql](/e/pg_graphql): GraphQL support in PostgreSQL (Rust extension via PIGSTY)
- [pg_jsonschema](/e/pg_jsonschema): JSON Schema validation (Rust extension via PIGSTY)
- [wrappers](/e/wrappers): Supabase’s foreign data source wrappers bundle (Rust extension via PIGSTY)
- [index_advisor](/e/index_advisor): Query index advisor (SQL extension via PIGSTY)
- [pg_net](/e/pg_net): Async non-blocking HTTP/HTTPS requests in SQL (C extension via PIGSTY)
- [vault](/e/supabase_vault): Store encrypted credentials in Vault (C extension via PIGSTY)
- [pgjwt](/e/pgjwt): PostgreSQL implementation of JSON Web Token API (SQL extension via PIGSTY)
- [pgsodium](/e/pgsodium): Table data encryption storage TDE (C extension via PIGSTY)
- [supautils](/e/supautils): Secure database clusters in cloud environments (C extension via PIGSTY)
- [pg_plan_filter](/e/plan_filter): Block specific queries using execution plan cost filtering (C extension via PIGSTY)
...
First, download & [install](/docs/install/start) pigsty as usual, with the `supa` config template:
...
-  [**Hardware**](/docs/prepare/hardware): necessary VM/BM resources, one node at least, 3-4 are recommended for HA.
-  [**Linux OS**](/docs/prepare/linux): Linux x86_64 server with fresh installed Linux, [check compatible distro](/docs/prepare/linux)
-  [**Network**](/docs/prepare/admin): Static IPv4 address which can be used as node identity
-  [**Admin User**](/docs/prepare/admin): nopass ssh & sudo are recommended for admin user
-  [**Conf Template**](/docs/config/): Use the [`supa`](/docs/config/template) config template, if you don’t know how to manually configure pigsty
