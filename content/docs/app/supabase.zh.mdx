...
[准备](/zh/prepare) [Linux](/zh/prepare/linux)，执行 Pigsty [标准安装](/zh/install/) 流程，选择 `supabase` 配置模板，依次执行：
...
而通过 Pigsty 自建的 Supabase 则提供了多达 [**423**](https://ext.pgsty.com/zh/list) 个开箱即用的 PG 扩展。

此外，自主可控与规避供应商锁定也是自建的重要原因 —— 尽管 Supabase 虽然旨在提供一个无供应商锁定的 Google Firebase 开源替代，但实际上自建高标准企业级的 Supabase 门槛并不低。
Supabase 内置了一系列由他们自己开发维护的 PG 扩展插件，并计划将原生的 PostgreSQL 内核替换为收购的 [OrioleDB](/zh/pgsql/kernel/orioledb)，而这些内核与扩展在 PGDG 官方仓库中并没有提供。

这实际上是某种隐性的供应商锁定，阻止了用户使用除了 supabase/postgres Docker 镜像之外的方式自建，Pigsty 则提供开源，透明，通用的方案解决这个问题。
我们将所有 Supabase 自研与用到的 10 个缺失的扩展打成开箱即用的 RPM/DEB 包，确保它们在所有 [主流Linux操作系统发行版](/zh/prepare/linux) 上都可用：

| 扩展                                                        | 说明                                                     |
|-----------------------------------------------------------|--------------------------------------------------------|
| [`pg_graphql`](https://ext.pgsty.com/e/pg_graphql/)       | 提供PG内的GraphQL支持 (RUST)，Rust扩展，由PIGSTY提供                |
| [`pg_jsonschema`](https://ext.pgsty.com/e/pg_jsonschema/) | 提供JSON Schema校验能力，Rust扩展，由PIGSTY提供                     |
| [`wrappers`](https://ext.pgsty.com/e/wrappers/)           | Supabase提供的外部数据源包装器捆绑包,，Rust扩展，由PIGSTY提供               |
| [`index_advisor`](https://ext.pgsty.com/e/index_advisor/) | 查询索引建议器，SQL扩展，由PIGSTY提供                                |
| [`pg_net`](https://ext.pgsty.com/e/pg_net/)               | 用 SQL 进行异步非阻塞HTTP/HTTPS 请求的扩展 (supabase)，C扩展，由PIGSTY提供 |
| [`vault`](https://ext.pgsty.com/e/supabase_vault/)        | 在 Vault 中存储加密凭证的扩展 (supabase)，C扩展，由PIGSTY提供            |
| [`pgjwt`](https://ext.pgsty.com/e/pgjwt/)                 | JSON Web Token API 的PG实现 (supabase)，SQL扩展，由PIGSTY提供    |
| [`pgsodium`](https://ext.pgsty.com/e/pgsodium/)           | 表数据加密存储 TDE，扩展，由PIGSTY提供                               |
| [`supautils`](https://ext.pgsty.com/e/supautils/)         | 用于在云环境中确保数据库集群的安全，C扩展，由PIGSTY提供                        |
| [`pg_plan_filter`](https://ext.pgsty.com/e/plan_filter/)  | 使用执行计划代价过滤阻止特定查询语句，C扩展，由PIGSTY提供                       |

同时，我们在 Supabase 自建部署中默认 [安装](/zh/pgsql/extension/install)绝大多数扩展，您可以参考可用扩展列表按需 [启用](/zh/pgsql/extension/create)。

同时，Pigsty 还会负责好底层 [高可用](/feat/ha/) [PostgreSQL](/zh/pgsql/) 数据库集群，高可用 [MinIO](/zh/minio/) 对象存储集群的自动搭建，甚至是 [Docker](/zh/docker/) 容器底座的部署与 [Nginx](/admin/portal) 反向代理，[域名配置](/zh/admin/domain) 与 [HTTPS证书签发](/zh/admin/cert)。 您可以使用 Docker Compose 拉起任意数量的无状态 Supabase 容器集群，并将状态存储在外部 Pigsty 自托管数据库服务中。

在这一自建部署架构中，您获得了使用不同内核的自由（PG 15-17，OrioleDB），加装 [**423**](https://ext.pgsty.com/list/) 个扩展的自由，扩容与伸缩 Supabase / Postgres / MinIO 的自由，
...
[准备](/zh/prepare) 一台全新 [Linux 服务器](/zh/prepare/linux)，使用 Pigsty 提供的 [`supabase`](https://github.com/pgsty/pigsty/blob/main/conf/supabase.yml) 配置模板执行 [标准安装](/zh/install/start)，
然后额外运行 [`docker.yml`](/zh/docker/playbook#dockeryml) 与 [`app.yml`](/zh/app/playbook) 拉起无状态部分的 Supabase 容器即可（默认端口 `8000`/`8433`）。
...
<Callout title="中国大陆地区 DockerHub 被墙" type="warning">
    ...
    你也可以自行配置 [代理](/zh/docker/config#proxy) 与 [镜像站](/zh/docker/config#registry) ，`cd /opt/supabase; docker compose pull` 手动拉取镜像。
    我们亦提供包含完整离线安装方案的 [Supabase 自建专家咨询服务](/zh/service)。
...
如果您的服务直接向公网暴露，我们强烈建议您使用真正的域名与 HTTPS 证书，并通过 [Nginx 门户](/zh/admin/portal) 访问。
...
对于严肃的生产部署，我们强烈建议您修改 [Pigsty 基础组件的密码](/zh/config/security#passwords)。因为这些默认值是公开且众所周知的，不改密码上生产无异于裸奔：

- [`grafana_admin_password`](/infra/param/#grafana_admin_password): `pigsty`，Grafana管理员密码
- [`pg_admin_password`](/pgsql/param/#pg_admin_password): `DBUser.DBA`，PG超级用户密码
- [`pg_monitor_password`](/pgsql/param/#pg_monitor_password): `DBUser.Monitor`，PG监控用户密码
- [`pg_replication_password`](/pgsql/param/#pg_replication_password): `DBUser.Replicator`，PG复制用户密码
- [`patroni_password`](/pgsql/param/#patroni_password): `Patroni.API`，Patroni 高可用组件密码
- [`haproxy_admin_password`](/node/param/#haproxy_admin_password): `pigsty`，负载均衡器管控密码
- [`minio_secret_key`](/minio/param/#minio_secret_key): `minioadmin`，MinIO 根用户密钥
...
完整的域名/HTTPS 配置可以参考 [证书管理](/zh/admin/cert) 教程，您也可以使用 Pigsty 自带的本地静态解析与自签发 HTTPS 证书作为下位替代。
...
Pigsty 会将备份仓库切换到外部对象存储上，更多备份配置可以参考 [PostgreSQL 备份](/zh/pgsql/backup) 文档。
...
这样的部署在故障时可以提供一个最低标准的 RTO （小时级恢复时长）/ RPO （MB级数据损失）[兜底容灾水平](/zh/pgsql/backup) 兜底。

如果想要达到 RTO < 30s ，切换零数据丢失，那么需要使用[多节点](/zh/install/multinode)进行高可用部署，这涉及到：

- [ETCD](/zh/etcd/)： DCS 需要使用三个节点或以上，才能容忍一个节点的故障。
- [PGSQL](/zh/pgsql/)： PGSQL 同步提交不丢数据模式，建议使用至少三个节点。
- [INFRA](/zh/infra/)：监控基础设施故障影响稍小，建议生产环境使用双副本
- Supabase 无状态容器本身也可以是多节点的副本，可以实现高可用。

在这种情况下，您还需要修改 PostgreSQL 与 MinIO 的接入点，使用 DNS / L2 VIP / HAProxy 等 [高可用接入点](/zh/pgsql/service#access)
