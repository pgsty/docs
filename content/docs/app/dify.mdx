...
On a fresh Linux x86 / ARM server running a [**compatible distribution**](/docs/prepare/linux), execute:
...
First, use Pigsty’s [standard installation process](/docs/install) to install the PostgreSQL instance required by Dify:
...
-  Hardware/Software: [Prepare required machine resources](/docs/prepare): Linux `x86_64/arm64` server, fresh installation of [mainstream Linux operating systems](/docs/prepare/linux)
-  Network/Permissions: [SSH](https://pigsty.io/docs/setup/prepar/#management-user-preparation) passwordless login access, user has [passwordless sudo privileges](/docs/prepare/admin#user)
