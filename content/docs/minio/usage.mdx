...
After MinIO cluster is [configured](https://pigsty.io/docs/minio/config/) and deployed with the [playbook](https://pigsty.io/docs/minio/playbook/), you can start using and accessing the MinIO cluster by following the instructions here.
...
Define it in the [config inventory](https://pigsty.io/docs/setup/config/#inventory), then run the playbook:

```bash
./minio.yml -l minio
```

The [`install.yml`](https://pigsty.io/docs/setup/playbook/#installyml) playbook will automatically create the MinIO cluster defined in the inventory, so you don’t need to run the `minio.yml` playbook manually, if you choose the default one-pass installation.

If you plan to deploy a production-grade large-scale multi-node MinIO cluster, we **strongly** recommend you to read the Pigsty MinIO [configuration document](https://pigsty.io/docs/minio/config/) and the MinIO [document](https://min.io/docs/minio/linux/operations/concepts.html) before proceeding.
...
1. You can add static resolution records in [`node_etc_hosts`](https://pigsty.io/docs/node/param/#node_etc_hosts) or manually modify the `/etc/hosts` file
2. You can add a record on the internal DNS server if you are using an DNS service
3. You can add a record in [`dns_records`](https://pigsty.io/docs/infra/param/#dns_records) if you are using the DNSMASQ on infra nodes
...
For example, if you are using haproxy to expose MinIO service [like this](https://pigsty.io/docs/minio/config/#expose-service), the port may be `9002`.
...
The MinIO is used as a backup repository for pgBackRest by default in Pigsty. When you modify the [`pgbackrest_method`](https://pigsty.io/docs/pgsql/param/#pgbackrest_method) to `minio`, the PGSQL module will automatically switch the backup repository to MinIO.
