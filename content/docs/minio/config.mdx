...
You have to define a MinIO cluster in the [config inventory](https://pigsty.io/docs/setup/config/#inventory) before deploying it.

There are 3 major deployment modes for MinIO clusters:

- [SNSD: Single-Node Single-Drive](https://pigsty.io/docs/minio/config/#single-node-single-drive): you can use any dir as driver in this mode, convenient for dev/test/demo.
- [SNMD: Single-Node Multi-Drive](https://pigsty.io/docs/minio/config/#single-node-multi-drive): a compromise mode, use multiple disks (>=2) on a single server, only when resources are extremely limited.
- [MNMD: Multi-Node Multi-Drive](https://pigsty.io/docs/minio/config/#multi-node-multi-drive): standard production deployment with the best reliability, but requires multiple servers and real drivers.
...
Besides, you can use [multi-pool deployment](https://pigsty.io/docs/minio/config/#multi-pool) to scale an existing MinIO cluster, or directly deploy [multiple clusters](https://pigsty.io/docs/minio/config/#multiple-clusters).

When using a multi-node MinIO cluster, you can access the service from any node, so the best practice is to use a load balancer and HA [access](https://pigsty.io/docs/minio/config/#access-service).
...
- SNSD: `MINIO_VOLUMES` is point to any dir on local node, from [`minio_data`](https://pigsty.io/docs/minio/param/#minio_data)
- SNMD: `MINIO_VOLUMES` is point to a series of real driver on local node, from [`minio_data`](https://pigsty.io/docs/minio/param/#minio_data)
- MNMD: `MINIO_VOLUMES` is point to multiple node & multiple drivers, according to  `minio_data`  and  `minio_node`

- Use [`minio_data`](https://pigsty.io/docs/minio/param/#minio_data) to specify drivers on each node, such as `/data{1...4}`
- Use [`minio_node`](https://pigsty.io/docs/minio/param/#minio_node) to specify node name pattern, such as `${minio_cluster}-${minio_seq}.pigsty`
...
The only required params are [`minio_seq`](https://pigsty.io/docs/minio/param/#minio_seq) and [`minio_cluster`](https://pigsty.io/docs/minio/param/#minio_cluster), which generate a unique identity for each MinIO instance.

Single-Node Single-Driver mode is for dev purposes, so you can use a common dir as the data dir. The default data dir for SNSD minio is specified by [`minio_data`](https://pigsty.io/docs/minio/param/#minio_data), which is `/data/minio` by default. Beware that in multi-driver or multi-node mode, MinIO will refuse to start if using a common dir as the data dir rather than a mount point.

We strongly recommend using a static domain name record to access MinIO. For example, the default `sss.pigsty` if [`minio_domain`](https://pigsty.io/docs/minio/param/#minio_domain) can be added to all nodes through:
...
To use multiple disks on a single node, you have to specify the [`minio_data`](https://pigsty.io/docs/minio/param/#minio_data) in the format of `{{ prefix }}{x...y}`, which defines a series of disk mount points.
...
The vagrant MinIO [sandbox](https://github.com/pgsty/pigsty/blob/main/vagrant/spec/minio.rb) has pre-defined 4-node MinIO cluster with 4 drivers. You have to properly mount them before starting MinIO (be sure to format disks with `xfs`):
...
The extra [`minio_node`](https://pigsty.io/docs/minio/param/#minio_node) param will be used for a multi-node deployment in addition to the [`minio_data`](https://pigsty.io/docs/minio/param/#minio_data)
...
MinIO’s architecture allows for [cluster expansion](https://pigsty.io/docs/minio/admin/#expand-cluster) by adding new storage pools. In Pigsty, you can achieve this by explicitly specifying the [`minio_volumes`](https://pigsty.io/docs/minio/param/#minio_volumes) param to specify nodes/disks for each pool.

For example, suppose you have already created a MinIO cluster as defined in the [Multi-Node Multi-Disk](https://pigsty.io/docs/minio/config/#multi-node-multi-drive) example, and now you want to add a new storage pool consisting of four nodes.

You can specify [`minio_volumes`](https://pigsty.io/docs/minio/param/#minio_volumes) here to allocate nodes for each pool to scale out the cluster.
...
For more information on storage pools, please refer to [Management Plan: MinIO Cluster Expansion](https://pigsty.io/docs/minio/admin/).
...
MinIO will serve on port `9000` by default. If a multi-node MinIO cluster is deployed, you can access its service via any node. It would be better to expose MinIO service via a load balancer, such as the default [`haproxy`](https://pigsty.io/docs/node/param/#haproxy) on [`NODE`](https://pigsty.io/docs/node/), or use the L2 [vip](https://pigsty.io/docs/node/param/#node_vip).

To expose MinIO service with haproxy, you have to define an extra service with [`haproxy_services`](https://pigsty.io/docs/node/param/#haproxy_services):
...
Service access falls under the scope of the [NODE](https://pigsty.io/docs/node/) module, and we’ll provide only a basic introduction here.

High-availability access to a multi-node MinIO cluster can be achieved using an L2 VIP or HAProxy. For example, you can use Keepalived to bind an L2 [VIP](https://pigsty.io/docs/node/param/#node_vip) to the MinIO cluster, or use the [`haproxy`](https://pigsty.io/docs/node/param/#haproxy) component provided by the [`NODE`](https://pigsty.io/docs/node/) module to expose MinIO services through a load balancer.
...
In this scenario, you may also need to globally modify the destination of domain name resolution and adjust the [`minio_endpoint`](https://pigsty.io/docs/minio/param/#minio_endpoint) parameter to change the endpoint address corresponding to the MinIO alias on the management node:
...
For example, the [`prod`](https://pigsty.io/docs/conf/prod) template uses this way.
...
To use the [exposed service](https://pigsty.io/docs/minio/config/#expose-service), you have to update/append the MinIO credential in the [`pgbackrest_repo`](https://pigsty.io/docs/pgsql/param/#pgbackrest_repo) section:
...
MinIO has a built-in console that can be accessed via HTTPS @ [`minio_admin_port`](https://pigsty.io/docs/minio/param/#minio_admin_port). If you want to expose the MinIO console to the outside world, you can add MinIO to [`infra_portal`](https://pigsty.io/docs/infra/param/#infra_portal).
