...
You have to [configure](https://pigsty.io/docs/minio/config/) minio cluster in the [config inventory](https://pigsty.io/docs/setup/config/#inventory) before running the playbook.

------

## Playbook

There’s a built-in playbook: [`minio.yml`](https://pigsty.io/docs/minio/playbook/#minioyml) for installing the MinIO cluster.
...
You should install [`MINIO`](https://pigsty.io/docs/minio/) module on Pigsty-managed nodes (i.e., Install [`NODE`](https://pigsty.io/docs/node/) first)
...
- [`minio_clean`](https://pigsty.io/docs/minio/param/#minio_clean), `false` by default, which means Pigsty will not remove existing MinIO data by default
