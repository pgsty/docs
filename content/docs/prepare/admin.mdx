...
<Callout title="Don't use postgres dbsu as admin user" type="error">
    ...
    If you are using a dfferent dbsu user, avoid using it as admin user as well.
...
The no password is optional if you can accept the password prompt for every `ssh` and `sudo` command.
...
<Callout title="Create admin user with pigsty" type="info">
    ...
    It will create a dedicate `dba` (uid=88) user described by the following parameters,
...
<Callout title="Allow sudo without password" type="info">
    ...
    To manually allow an user to execute `sudo` commands without password:
