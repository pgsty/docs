...
Before deploying a Mongo (FerretDB) cluster, you need to define it in the inventory using the relevant [parameters](https://pigsty.io/docs/ferret/config/#parameters).
...
FerretDB 2.0+ requires an extension: [DocumentDB](https://pigsty.io/ext/sim/documentdb/), which depends on several other extensions. Here’s a template for creating a PostgreSQL cluster for FerretDB:
...
You can use [Services](https://pigsty.io/docs/concept/svc/) to connect to a highly available PostgreSQL cluster and deploy multiple FerretDB instance replicas with L2 VIP binding for FerretDB layer high availability.
