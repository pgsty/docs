...
[Prepare](/prepare) a [Linux](/prepare/linux) server, follow the Pigsty [standard installation](/install/) process, select the `supabase` configuration template, and execute the following commands:
...
Self-built Supabase with Pigsty provides up to [**437**](https://pgext.cloud/list) ready-to-use PostgreSQL extensions.
...
Supabase includes a series of PostgreSQL extension plugins developed and maintained by them, and plans to replace the native PostgreSQL kernel with the acquired [OrioleDB](/pgsql/kernel/orioledb), but these kernels and extensions are not provided in the official PGDG repository.
...
We package all 10 missing extensions developed and used by Supabase into ready-to-use RPM/DEB packages, ensuring they are available on all [mainstream Linux operating system distributions](/prepare/linux):
...
Meanwhile, we [install](/pgsql/extension/install) most extensions by default in Supabase self-hosting deployment. You can refer to the available extension list to [enable](/pgsql/extension/create) them as needed.

Additionally, Pigsty handles the automatic setup of underlying [high availability](/feat/ha/) [PostgreSQL](/pgsql/) database clusters, high availability [MinIO](/minio/) object storage clusters, and even [Docker](/docker/) container infrastructure deployment and [Nginx](/admin/portal) reverse proxy, [domain configuration](/admin/domain) and [HTTPS certificate issuance](/admin/cert). You can deploy any number of stateless Supabase container clusters using Docker Compose and store state in external Pigsty self-hosted database services.

In this self-hosting deployment architecture, you gain the freedom to use different kernels (PostgreSQL 15-18, OrioleDB), the freedom to install [**437**](https://pgext.cloud/list/) extensions, the freedom to scale Supabase/Postgres/MinIO,
...
[Prepare](/prepare) a fresh [Linux server](/prepare/linux), use the [`supabase`](https://github.com/pgsty/pigsty/blob/main/conf/supabase.yml) configuration template provided by Pigsty to execute the [standard installation](/install/start) process,
then additionally run [`docker.yml`](/docker/playbook#dockeryml) and [`app.yml`](/app/playbook) to deploy the stateless Supabase containers (default ports `8000`/`8433`).
...
[![asciicast](/img/asciinema/supabase.svg)](https://asciinema.org/a/731206)
...
<Callout title="DockerHub blocked in mainland China" type="warning">
    ...
    You can also configure [proxy](/docker/config#proxy) and [mirror sites](/docker/config#registry) yourself, or manually pull images with `cd /opt/supabase; docker compose pull`.
    We also provide [Supabase self-hosting expert consulting services](/service) including complete offline installation solutions.
...
If your service is directly exposed to the public network, we strongly recommend using real domains and HTTPS certificates and accessing through [Nginx Portal](/admin/portal).
...
For serious production deployments, we strongly recommend changing [Pigsty default passwords](/config/security#passwords).
...
- [`pg_admin_password`](/pgsql/param/#pg_admin_password): `DBUser.DBA`, PostgreSQL superuser password
- [`pg_monitor_password`](/pgsql/param/#pg_monitor_password): `DBUser.Monitor`, PostgreSQL monitoring user password
- [`pg_replication_password`](/pgsql/param/#pg_replication_password): `DBUser.Replicator`, PostgreSQL replication user password
- [`patroni_password`](/pgsql/param/#patroni_password): `Patroni.API`, Patroni high availability component password
- [`haproxy_admin_password`](/node/param/#haproxy_admin_password): `pigsty`, load balancer management password
- [`minio_secret_key`](/minio/param/#minio_secret_key): `minioadmin`, MinIO root user key
...
- [`JWT_SECRET`](https://github.com/pgsty/pigsty/blob/main/conf/supabase.yml#L131)
...
- [`DASHBOARD_USERNAME`](https://github.com/pgsty/pigsty/blob/main/conf/supabase.yml#L136): Supabase Studio Web interface default username, default is `supabase`
- [`DASHBOARD_PASSWORD`](https://github.com/pgsty/pigsty/blob/main/conf/supabase.yml#L137): Supabase Studio Web interface default password, default is `pigsty`
...
Complete domain/HTTPS configuration can refer to the [Certificate Management](/admin/cert) tutorial. You can also use Pigsty's built-in local static resolution and self-signed HTTPS certificates as fallback.

[![asciicast](/img/asciinema/supa-domain.svg)](https://asciinema.org/a/731211)
...
Pigsty will switch the backup repository to external object storage. More backup configurations can refer to [PostgreSQL Backup](/pgsql/backup) documentation.
...
Such deployment can provide a minimum standard RTO (hour-level recovery time)/RPO (MB-level data loss) [fallback disaster recovery level](/pgsql/backup) during failures.

To achieve RTO < 30s with zero data loss failover, you need to use [multi-node](/install/multinode) high availability deployment, which involves:

- [ETCD](/etcd/): DCS needs three or more nodes to tolerate one node failure.
- [PGSQL](/pgsql/): PostgreSQL synchronous commit mode without data loss, recommend using at least three nodes.
- [INFRA](/infra/): Monitoring infrastructure failure has less impact, recommend using dual replicas in production
- Supabase stateless containers themselves can also be multi-node replicas to achieve high availability.

In this case, you also need to modify PostgreSQL and MinIO access points to use DNS/L2 VIP/HAProxy and other [high availability access points](/pgsql/service#access)
