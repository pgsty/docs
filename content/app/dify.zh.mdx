...
在运行 [**兼容发行版**](/zh/prepare/linux) 的全新 Linux x86 / ARM 服务器上执行：
...
Pigsty 确保所有 Dify 状态都存储在外部管理的服务中，包括 PostgreSQL 中的元数据和文件系统中的其他数据。因此，通过 Docker Compose 启动的 Dify 实例成为可以随时销毁和重建的无状态应用程序，大大简化了运维。
...
首先，使用 Pigsty 的 [标准安装过程](/zh/install) 安装 Dify 所需的 PostgreSQL 实例：
...
当您使用 `./configure -c app/dify` 命令时，Pigsty 会根据 [`conf/app/dify.yml`](https://github.com/pgsty/pigsty/blob/main/conf/app/dify.yml) 模板和您当前的环境自动生成配置文件。您应该根据实际需要在生成的 `pigsty.yml` 配置文件中修改密码、域名和其他相关参数，然后使用 `./install.yml` 执行标准安装过程。
...
- 硬件/软件：[准备所需的机器资源](/zh/prepare/hardware)：Linux `x86_64/arm64` 服务器，[主流 Linux 操作系统](/zh/prepare/linux) 的全新安装
- 网络/权限：[SSH](/zh/prepare/admin#ssh) 免密登录访问权限，用户具有 [免密 sudo 权限](/zh/prepare/admin#user)
...
- 您是否修改了所有密码相关的配置参数？【可选】
  ...
  - [`pg_admin_password`](/zh/pgsql/param/#pg_admin_password)：`DBUser.DBA`，PG 超级用户密码
  - [`pg_monitor_password`](/zh/pgsql/param/#pg_monitor_password)：`DBUser.Monitor`，PG 监控用户密码
  - [`pg_replication_password`](/zh/pgsql/param/#pg_replication_password)：`DBUser.Replicator`，PG 复制用户密码
  - [`patroni_password`](/zh/pgsql/param/#patroni_password)：`Patroni.API`，Patroni HA 组件密码
  - [`haproxy_admin_password`](/zh/node/param/#haproxy_admin_password)：`pigsty`，负载均衡器管理密码
...
- [`infra_portal`](/zh/infra/param/#infra_portal) 参数的 `dify` 域名
- 最好指定一个邮箱地址 [`certbot_email`](/zh/infra/param/#certbot_email) 用于接收证书过期通知
...
您可以使用 `restic` 备份 Dify 的文件系统。Dify 的数据文件在 `/data/dify` 目录中。您可以使用以下命令进行备份：
