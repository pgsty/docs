...
[**pig**](https://ext.pgsty.com/pig) 是一个开源的 PostgreSQL（和扩展）包管理器，支持 [主流 Linux](/zh/prepare/linux) 发行版。

在 (`amd64` / `arm64`) 上使用原生 `apt`/`yum`/`dnf` 安装 PostgreSQL 13~17 以及 [423 个扩展](https://ext.pgsty.com/list)。

最新稳定版本是 [v0.6.0](https://github.com/pgsty/pig/releases/tag/v0.6.0)，
查看 [GitHub 仓库](https://github.com/pgsty/pig) 和 [pig 文档](https://ext.pgsty.com/pig) 了解更多详情。
...
- 全新的扩展目录网站：https://ext.pgsty.com
- 将扩展列表更新至 423 个（包括 pg_tde / orioledb）
- 添加 percona postgres 内核别名（支持 pg_tde）
- 新子命令 `pig install`，是 `pig sty install` 的包装器
- 更新扩展信息输出
