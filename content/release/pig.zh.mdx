---
title: PIG 发布
description: pig 包管理器发布说明
icon: PiggyBank
---

[**pig**](https://pgext.cloud/pig) 是一个开源的 PostgreSQL（和扩展）包管理器，支持 [主流 Linux](/zh/prepare/linux) 发行版。

在 (`amd64` / `arm64`) 上使用原生 `apt`/`yum`/`dnf` 安装 PostgreSQL 13~18 以及 [437 个扩展](https://pgext.cloud/list)。

最新稳定版本是 [v0.7.3](https://github.com/pgsty/pig/releases/tag/v0.7.3)，
查看 [GitHub 仓库](https://github.com/pgsty/pig) 和 [pig 文档](https://pgext.cloud/pig) 了解更多详情。
...
[`pig`](/zh/pig/) 项目现在有了新的 [主页](https://pgext.cloud/pig)，以及 PostgreSQL 扩展 [目录](https://pgext.cloud/list)。
...
您可以使用简单的命令安装 PostgreSQL 内核以及 [**404 个扩展**](https://pgext.cloud/list)。此外，
pig v0.3 也嵌入并随最新的 Pigsty [v3.3.0](https://doc.pgsty.com/release) 一起发布。

**新功能**

`pig build` 子命令具有设置扩展构建环境的 [能力](https://ext.pgsty.com/cmd/build/)
...
[`pgext.cloud`](https://pgext.cloud/) 目录正在迁移到 [`https://pgext.cloud/list`](https://pgext.cloud/list)，包含更多信息！
...
- 新扩展：[`Omnigres`](https://pgext.cloud/e/omni) 33 个扩展，postgres 作为平台
- 新扩展：[`pg_mooncake`](https://pgext.cloud/e/pg_mooncake)：postgres 中的 duckdb
- 新扩展：[`pg_xxhash`](https://pgext.cloud/e/xxhash)
- 新扩展：[`timescaledb_toolkit`](https://pgext.cloud/e/timescaledb_toolkit)
- 新扩展：[`pg_xenophile`](https://pgext.cloud/e/pg_xenophile)
- 新扩展：[`pg_drop_events`](https://pgext.cloud/e/pg_drop_events)
- 新扩展：[`pg_incremental`](https://pgext.cloud/e/pg_incremental)
...
- 将 [`pg_extra_time`](https://pgext.cloud/e/pg_extra_time) 升级到 2.0.0
- 将 [`pg_vectorize`](https://pgext.cloud/e/pg_vectorize) 升级到 0.20.0
...
查看高级用法详情和 [列出 340 个可用扩展](https://pgext.cloud/list)。
...
- [`citus`](https://pgext.cloud/e/citus) 在 `aarch64` 和 ubuntu 24.04 上不可用
- [`pljava`](https://pgext.cloud/e/pljava) 在 `el8` 上缺失
- [`jdbc_fdw`](https://pgext.cloud/e/jdbc_fdw) 在 `el8.aarch64` 和 `el9.aarch64` 上缺失
- [`pllua`](https://pgext.cloud/e/pllua) 在 `el8.aarch64` 上对 pg 13,14,15 缺失
- [`topn`](https://pgext.cloud/e/topn) 在 `el8.aarch64` 和 `el9.aarch64` 上对 pg13 缺失，以及所有 `deb.aarch64`
- [`pg_partman`](https://pgext.cloud/e/pg_partman) 和 [`timeseries`](https://pgext.cloud/e/timeseries) 在 `u24` 上对 pg13 缺失
- [`wiltondb`](https://pgext.cloud/e/babelfishpg_common) 在 `d12` 上缺失
