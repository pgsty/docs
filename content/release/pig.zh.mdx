...
[**pig**](https://pig.pgsty.com) 是一个开源的 PostgreSQL（和扩展）包管理器，支持 [主流 Linux](/zh/prepare/linux) 发行版。

在 (`amd64` / `arm64`) 上使用原生 `apt`/`yum`/`dnf` 安装 PostgreSQL 13~17 以及 [437 个扩展](https://ext.pgsty.com/list)。

最新稳定版本是 [v0.7.3](https://github.com/pgsty/pig/releases/tag/v0.7.3)，
查看 [GitHub 仓库](https://github.com/pgsty/pig) 和 [pig 文档](https://pig.pgsty.com) 了解更多详情。
