---
title: PIG Release
description: the pig package manager release note
icon: PiggyBank
---

[**pig**](https://pgext.cloud/pig) is an open-source PostgreSQL (& Extension) Package Manager for [Mainstream Linux](/prepare/linux).

Install PostgreSQL 13 ~ 18 along with [437 extensions](https://pgext.cloud/list) on (`amd64` / `arm64`) with native `apt`/`yum`/`dnf`.

The latest stable version is [v0.7.4](https://github.com/pgsty/pig/releases/tag/v0.7.4),
Check the [GitHub Repo](https://github.com/pgsty/pig) and [pig documentation](https://pgext.cloud/pig) for more details.

|     Version     |    Date    | Summary                                    |                           GitHub                           |
|:---------------:|:----------:|--------------------------------------------|:----------------------------------------------------------:|
| [v0.7.4](#v074) | 2025-12-01 | update ivory, pgtde, pgdg extras           | [v0.7.4](https://github.com/pgsty/pig/releases/tag/v0.7.4) |
...
- New extension catalog: [https://pgext.cloud](https://pgext.cloud)
...
The [`pig`](/pig/) project now has a new [homepage](https://pgext.cloud/pig), alone with the PostgreSQL Extension [Catalog](https://pgext.cloud/list).
...
You can install PostgreSQL Kernels along with & [**404 extensions**](https://pgext.cloud/list) with a simple command. Besides,
pig v0.3 is also embedded & shipped with the latest Pigsty [v3.3.0](https://doc.pgsty.com/release).

**New Features**

`pig build` subcommand with the [ability](https://pgext.cloud/pig/cmd/build/) to set up extension building environment
...
The [`pgext.cloud`](https://pgext.cloud/) catalog is moving to [`https://pgext.cloud`](https://pgext.cloud) with more information!
...
Check the advanced usage for details and [list 340 available extensions](https://pgext.cloud/list).
...
- [`citus`](https://pgext.cloud/e/citus) is not available on `aarch64` and ubuntu 24.04
- [`pljava`](https://pgext.cloud/e/pljava) is missing on `el8`
- [`jdbc_fdw`](https://pgext.cloud/e/jdbc_fdw) is missing on `el8.aarch64` and `el9.aarch64`
- [`pllua`](https://pgext.cloud/e/pllua) is missing on `el8.aarch64` for pg 13,14,15
- [`topn`](https://pgext.cloud/e/topn) is missing on `el8.aarch64` and `el9.aarch64` for pg13, and all `deb.aarch64`
- [`pg_partman`](https://pgext.cloud/e/pg_partman) and [`timeseries`](https://pgext.cloud/e/timeseries) is missing on `u24` for pg13
- [`wiltondb`](https://pgext.cloud/e/babelfishpg_common) is missing on `d12`
