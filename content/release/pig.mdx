...
[**pig**](https://ext.pgsty.com/pig) is an open-source PostgreSQL (& Extension) Package Manager for [Mainstream Linux](/prepare/linux).

Install PostgreSQL 13~17 along with [423 extensions](https://ext.pgsty.com/list) on (`amd64` / `arm64`) with native `apt`/`yum`/`dnf`.

The latest stable version is [v0.6.0](https://github.com/pgsty/pig/releases/tag/v0.6.0),
Check the [GitHub Repo](https://github.com/pgsty/pig) and [pig documentation](https://ext.pgsty.com/pig) for more details.
