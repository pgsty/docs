...
[**pig**](https://pig.pgsty.com) is an open-source PostgreSQL (& Extension) Package Manager for [Mainstream Linux](/prepare/linux).

Install PostgreSQL 13~17 along with [437 extensions](https://pgext.cloud/list) on (`amd64` / `arm64`) with native `apt`/`yum`/`dnf`.

The latest stable version is [v0.6.0](https://github.com/pgsty/pig/releases/tag/v0.6.0),
Check the [GitHub Repo](https://github.com/pgsty/pig) and [pig documentation](https://pig.pgsty.com) for more details.
...
The [`pig`](/pig/) project now has a new [homepage](https://pig.pgsty.com), alone with the PostgreSQL Extension [Catalog](https://pgext.cloud/list).
