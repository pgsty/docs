...
4. 确认是整集群原地 [PITR 回滚](/zh/pgsql/backup/restore)，还是新开服务器重放，还是用从库来重放，并执行恢复策略


## 误删集群

如果出现整个数据库集群通过 Pigsty 管理命令被误删的情况，例如错误的执行 [`pgsql-rm.yml`](/zh/pgsql/playbook#pgsql-rmyml) 剧本或 `bin/pgsql-rm` 命令。
除非您指定了 [`pg_rm_backup`](/zh/pgsql/param#pg_rm_backup) 参数为 `false`，否则备份会与数据库集群一起被删除。
