...
在极端的情况下，我们建议您也需要至少每周查阅一次监控，关注出现的告警事件，这样做可以提前规避绝大多数故障与问题。

这里列举了 Pigsty 中预先定义的 [告警规则](https://p.pgsty.com/alerts) 列表。
...
然而对于生产环境的多节点集群而言，您仍然需要在合适的时机（例如第二天工作日），进行善后工作，包括：
...
您可以首先通过 Pigsty 的 [PGCAT Database - Table Bloat](https://g.pgsty.com/d/pgcat-database?viewPanel=panel-190) 面板，确认数据库中的表膨胀情况与索引膨胀情况。
并选择膨胀率较高（建议优先处理那些膨胀率高于 50% 的较大表）的表与索引，使用 `pg_repack` 进行在线重整：

```bash
pg_repack dbname -t schema.table
```

重整期间不会影响正常写入，但是在重整完毕之后的瞬间需要获取表上的最强 AccessExclusive 锁以完成表更名替换。
