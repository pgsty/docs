...
You can use the parameters in [`pg_packages`](#pg_packages) and [`pg_extensions`](#pg_extensions) to install rpm/deb for the specific pg major version.
...
The full extension list can be found in [`roles/node_id/vars`](https://github.com/pgsty/pigsty/blob/main/roles/node_id/vars/) and listed in [Extension List](https://pgext.cloud/list/).
...
The only 3rd party extension is [`pg_repack`](https://pgext.cloud/e/pg_repack/), which is important for database maintenance, all other extensions are built-in postgres contrib extensions.
...
This should be set in pair with  [`prometheus_scrape_interval`](/zh/infra/param#prometheus_scrape_interval)
...
but do not overwrite [`pgbouncer_exporter_port`](#pgbouncer_exporter_port) here.
