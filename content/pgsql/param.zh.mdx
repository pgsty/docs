...
| [`pg_fs_main`](#pg_fs_main)                         | `path`     | C   | postgres 备份数据目录，默认为 `/data/postgres`                   |
...
| [`pg_rpo`](#pg_rpo)                                 | `int`      | C   | 恢复点目标（字节），默认最多 `1MiB`                                  |
| [`pg_libs`](#pg_libs)                               | `string`   | C   | 预加载库，默认为 `timescaledb,pg_stat_statements,auto_explain` |
| [`pg_delay`](#pg_delay)                             | `interval` | I   | 备集群领导者的复制应用延迟                                          |
| [`pg_checksum`](#pg_checksum)                       | `bool`     | C   | 为 postgres 集群启用数据校验和？                                  |
| [`pg_pwd_enc`](#pg_pwd_enc)                         | `enum`     | C   | 密码加密算法：md5,scram-sha-256                               |
| [`pg_encoding`](#pg_encoding)                       | `enum`     | C   | 数据库集群编码，默认为 `UTF8`                                     |
| [`pg_locale`](#pg_locale)                           | `enum`     | C   | 数据库集群区域设置，默认为 `C`                                      |
| [`pg_lc_collate`](#pg_lc_collate)                   | `enum`     | C   | 数据库集群排序规则，默认为 `C`                                      |
| [`pg_lc_ctype`](#pg_lc_ctype)                       | `enum`     | C   | 数据库字符类型，默认为 `en_US.UTF8`                               |
| [`pgsodium_key`](#pgsodium_key)                     | `string`   | C   | pgsodium 密钥，64位十六进制数字，如果未指定则默认为 sha256(pg_cluster)     |
| [`pgsodium_getkey_script`](#pgsodium_getkey_script) | `path`     | C   | pgsodium getkey 脚本路径，默认为模板中的 `pgsodium_getkey`         |


[`PG_PROVISION`](#pg_provision)：创建用户、数据库和数据库内对象

| 名称                                                  | 类型            | 级别  | 注释                                  |
|-----------------------------------------------------|---------------|-----|-------------------------------------|
| [`pg_provision`](#pg_provision)                     | `bool`        | C   | 引导后置备 postgres 集群                   |
| [`pg_init`](#pg_init)                               | `string`      | G/C | 集群模板的置备初始化脚本，默认为 `pg-init`           |
| [`pg_default_roles`](#pg_default_roles)             | `role[]`      | G/C | postgres 集群中的默认角色和用户                |
| [`pg_default_privileges`](#pg_default_privileges)   | `string[]`    | G/C | 管理员用户创建时的默认权限                        |
| [`pg_default_schemas`](#pg_default_schemas)         | `string[]`    | G/C | 要创建的默认模式                             |
| [`pg_default_extensions`](#pg_default_extensions)   | `extension[]` | G/C | 要创建的默认扩展                             |
| [`pg_reload`](#pg_reload)                           | `bool`        | A   | hba 更改后重新加载 postgres                |
| [`pg_default_hba_rules`](#pg_default_hba_rules)     | `hba[]`       | G/C | postgres 默认基于主机的身份验证规则              |
| [`pgb_default_hba_rules`](#pgb_default_hba_rules)   | `hba[]`       | G/C | pgbouncer 默认基于主机的身份验证规则             |

[`PG_BACKUP`](#pg_backup)：使用 pgbackrest 设置备份仓库

| 名称                                          | 类型     | 级别  | 注释                                                                         |
|---------------------------------------------|--------|-----|----------------------------------------------------------------------------|
| [`pgbackrest_enabled`](#pgbackrest_enabled) | `bool` | C   | 在 pgsql 主机上启用 pgbackrest？                                                 |
| [`pgbackrest_clean`](#pgbackrest_clean)     | `bool` | C   | 初始化期间删除 pg 备份数据？                                                          |
| [`pgbackrest_log_dir`](#pgbackrest_log_dir) | `path` | C   | pgbackrest 日志目录，默认为 `/pg/log/pgbackrest`                                  |
| [`pgbackrest_method`](#pgbackrest_method)   | `enum` | C   | pgbackrest 仓库方法：local,minio,等…                                             |
| [`pgbackrest_repo`](#pgbackrest_repo)       | `dict` | G/C | pgbackrest 仓库配置：https://pgbackrest.org/configuration.html#section-repository |

[`PG_ACCESS`](#pg_access)：暴露 pg 服务，绑定 VIP 和注册 DNS

| 名称                                                    | 类型          | 级别  | 注释                                                  |
|-------------------------------------------------------|-------------|-----|-----------------------------------------------------|
| [`pgbouncer_enabled`](#pgbouncer_enabled)             | `bool`      | C   | 如果禁用，pgsql 主机上将不会启动 pgbouncer                       |
...
| [`pgbouncer_auth_query`](#pgbouncer_auth_query)       | `bool`      | C   | 查询 postgres 以检索未列出的业务用户？                            |
| [`pgbouncer_poolmode`](#pgbouncer_poolmode)           | `enum`      | C   | 连接池模式：transaction,session,statement，默认为 transaction |
| [`pgbouncer_sslmode`](#pgbouncer_sslmode)             | `enum`      | C   | pgbouncer 客户端 ssl 模式，默认禁用                           |
| [`pgbouncer_ignore_param`](#pgbouncer_ignore_param)   | `string[]`  | G/C | pgbouncer 启动时忽略这些参数                                 |
| [`pg_weight`](#pg_weight)                             | `int`       | I   | 服务中的相对负载均衡权重，默认为 100，范围 0-255                       |
| [`pg_service_provider`](#pg_service_provider)         | `string`    | G/C | 专用 haproxy 节点组名称，或默认为本地节点的空字符串                      |
| [`pg_default_service_dest`](#pg_default_service_dest) | `enum`      | G/C | 如果 svc.dest='default' 时的默认服务目标                      |
| [`pg_default_services`](#pg_default_services)         | `service[]` | G/C | postgres 默认服务定义                                     |
| [`pg_vip_enabled`](#pg_vip_enabled)                   | `bool`      | C   | 为 pgsql 主库启用 L2 VIP？默认为 false                       |
| [`pg_vip_address`](#pg_vip_address)                   | `cidr4`     | C   | `<ipv4>/<mask>` 格式的 VIP 地址，如果启用 VIP 则必需             |
| [`pg_vip_interface`](#pg_vip_interface)               | `string`    | C/I | VIP 监听的网络接口，默认为 eth0                                |
| [`pg_dns_suffix`](#pg_dns_suffix)                     | `string`    | C   | pgsql dns 后缀，默认为 ''                                 |
| [`pg_dns_target`](#pg_dns_target)                     | `enum`      | C   | auto, primary, vip, none, 或自定义 IP 地址                |

[`PG_MONITOR`](#pg_monitor)：为 PGSQL 实例添加监控

| 名称                                                              | 类型       | 级别 | 注释                                          |
|-----------------------------------------------------------------|----------|----|---------------------------------------------|
| [`pg_exporter_enabled`](#pg_exporter_enabled)                   | `bool`   | C  | 在 pgsql 主机上启用 pg_exporter？                  |
| [`pg_exporter_config`](#pg_exporter_config)                     | `string` | C  | pg_exporter 配置文件名                           |
| [`pg_exporter_cache_ttls`](#pg_exporter_cache_ttls)             | `string` | C  | pg_exporter 收集器 ttl 阶段（秒），默认为 '1,10,60,300' |
| [`pg_exporter_port`](#pg_exporter_port)                         | `port`   | C  | pg_exporter 监听端口，默认为 9630                   |
| [`pg_exporter_params`](#pg_exporter_params)                     | `string` | C  | pg_exporter dsn 的额外 URL 参数                  |
| [`pg_exporter_url`](#pg_exporter_url)                           | `pgurl`  | C  | 如果指定，覆盖自动生成的 pg dsn                         |
| [`pg_exporter_auto_discovery`](#pg_exporter_auto_discovery)     | `bool`   | C  | 启用自动数据库发现？默认启用                              |
| [`pg_exporter_exclude_database`](#pg_exporter_exclude_database) | `string` | C  | 自动发现期间不会监控的数据库的 CSV 列表                      |
| [`pg_exporter_include_database`](#pg_exporter_include_database) | `string` | C  | 自动发现期间将被监控的数据库的 CSV 列表                      |
| [`pg_exporter_connect_timeout`](#pg_exporter_connect_timeout)   | `int`    | C  | pg_exporter 连接超时（毫秒），默认为 200                |
| [`pg_exporter_options`](#pg_exporter_options)                   | `arg`    | C  | 覆盖 pg_exporter 的额外选项                        |
| [`pgbouncer_exporter_enabled`](#pgbouncer_exporter_enabled)     | `bool`   | C  | 在 pgsql 主机上启用 pgbouncer_exporter？           |
| [`pgbouncer_exporter_port`](#pgbouncer_exporter_port)           | `port`   | C  | pgbouncer_exporter 监听端口，默认为 9631            |
| [`pgbouncer_exporter_url`](#pgbouncer_exporter_url)             | `pgurl`  | C  | 如果指定，覆盖自动生成的 pgbouncer dsn                  |
| [`pgbouncer_exporter_options`](#pgbouncer_exporter_options)     | `arg`    | C  | 覆盖 pgbouncer_exporter 的额外选项                 |
| [`pgbackrest_exporter_enabled`](#pgbackrest_exporter_enabled)   | `bool`   | C  | 在 pgsql 主机上启用 pgbackrest_exporter？          |
| [`pgbackrest_exporter_port`](#pgbackrest_exporter_port)         | `port`   | C  | pgbackrest_exporter 监听端口，默认为 9854           |
| [`pgbackrest_exporter_options`](#pgbackrest_exporter_options)   | `arg`    | C  | pgbackrest_exporter 的额外 CLI 参数              |


[`PG_REMOVE`](#pg_remove)：删除 Postgres 集群

| 名称                              | 类型     | 级别    | 注释                                         |
|---------------------------------|--------|-------|--------------------------------------------|
| [`pg_safeguard`](#pg_safeguard) | `bool` | G/C/A | 如果启用 pg_safeguard，停止运行 pg_remove，默认为 false |
| [`keep_data`](#keep_data)       | `bool` | G/C/A | 删除期间保留 postgres 数据？默认为 false               |
| [`keep_backup`](#keep_backup)   | `bool` | G/C/A | 删除期间保留 pgbackrest 备份？默认为 false             |
| [`keep_package`](#keep_package) | `bool` | G/C/A | 删除期间保留 postgres 包？默认为 false                |
...
以下是一些常用的参数，用于标识 PGSQL [实体](/zh/pgsql/config#identity-parameters)：实例、服务等…
...
如果 `pg_mode` 设置为 `citus` 或 `gpsql`，则需要 [`pg_shard`](#pg_shard) 和 [`pg_group`](#pg_group) 用于水平分片集群。
...
[备份集群](/zh/pgsql/config/#standby-cluster)或级联从库的上游实例 IP 地址。

在集群的 `primary` 实例上设置 `pg_upstream` ，表示此集群是一个[备份集群](/zh/pgsql/config/#standby-cluster)，该实例将作为 `standby leader`，从上游集群接收并应用更改。
...
额外用于[监控](/zh/docs/pgsql/monitor/)远程 PostgreSQL 实例的 Exporter 定义，默认值：`{}`

如果您希望监控远程 PostgreSQL 实例，请在监控系统所在节点（Infra节点）集群上的 `pg_exporters` 参数中定义它们，并使用 [`pgsql-monitor.yml`](/zh/pgsql/playbook/#pgsql-monitoryml) 剧本来完成部署。
...
[默认](/zh/pgsql/acl/#默认用户)的数据库用户及其凭据，强烈建议在生产环境中修改这些用户的密码。
...
每一个数组元素都是一个 [用户/角色](/zh/pgsql/user) 定义，例如：
...
每一个数组元素都是一个 [业务数据库](/zh/pgsql/db/) 定义，例如：
...
用于在数据库集群层面定义额外的服务，数组中的每一个对象定义了一个[服务](/zh/pgsql/svc/#定义服务)，一个完整的服务定义样例如下：
...
对象数组，每一个对象都代表一条规则， [hba](/zh/pgsql/hba/#定义hba) 规则对象的定义形式如下：
...
此参数与 [`pg_hba_rules`](#pg_hba_rules) 基本类似，都是 [hba](/zh/pgsql/hba/#定义hba) 规则对象的数组，区别在于本参数是为 Pgbouncer 准备的。
...
如果任何参与交换的实例没有 [`pg_dbsu`](#pg_dbsu) 用户，与该实例相关的密钥交换将失败，但不影响其他实例的密钥交换。
...
更多详细信息，请查看 [PGSQL 文件结构](/zh/reference/fhs/#postgres-fhs)。
...
PostgreSQL 日志目录，默认为：`/pg/log/postgres`，[Promtail](/zh/node/param/#promtail) 会使用此变量收集 PostgreSQL 日志。
...
您也可以直接指定最终安装的 RPM/DEB 包名称，包名中的 `${pg_version}` 或 `$v` 版本号占位符将被替换为具体的大版本号 [`pg_version`](#pg_version) 。
...
本参数在形式上与 [`pg_packages`](#pg_packages) 一致，但是通常用于指定需要安装的扩展插件，而且在这里指定的软件包会升级到可用的最新版本。
...
完整列表请参考：[`roles/node_id/vars`](https://github.com/pgsty/pigsty/blob/main/roles/node_id/vars/) 与 Pigsty [扩展目录](https://pgext.cloud/list/)
...
这是一个指向底层实际数据目录的符号链接，在多处被使用，请不要修改它。参阅 [PGSQL文件结构](/zh/reference/fhs/) 获取详细信息。
...
在 [etcd](/zh/etcd/) 中使用的顶级命名空间，由 patroni 和 vip-manager 使用，默认值是：`/pg`，不建议更改。
...
patroni日志目录，默认为`/pg/log/patroni`，由[`promtail`](/zh/node/param/#promtail)收集。
...
请注意，如果您的所有访问流量都使用 HAproxy 健康检查[服务接入](/zh/pgsql/svc/#接入服务)，正常是不存在脑裂风险的。
...
Patroni的危险 REST API （比如重启集群）由额外的用户名/密码保护，查看[配置集群](/zh/pgsql/admin/#配置集群)和[Patroni RESTAPI](https://patroni.readthedocs.io/en/latest/rest_api.html)以获取详细信息。
...
默认值：`oltp.yml`，但是[配置](/zh/setup/install/#配置)程序将在当前节点为小节点时将此值设置为 `tiny.yml`。
...
默认值：`0.25`，意味着节点内存的 25% 将被用作 PostgreSQL 的分片缓冲区。如果您想为 PostgreSQL 启用大页，那么此参数值应当适当小于 [`node_hugepage_ratio`](/zh/node/param/#node_hugepage_ratio)。
...
对于现有集群，您可以直接[配置集群](/zh/pgsql/admin/#配置集群)的 `shared_preload_libraries` 参数并应用生效。
...
查看 [延迟备用集群](/zh/pgsql/config/#延迟集群) 以获取详细信息。
...
通常应当与 [`pg_lc_collate`](#pg_lc_collate) 和 [`pg_lc_ctype`](#pg_lc_ctype) 配置保持一致。
...
配置规则与 [`pg_locale`](#pg_locale) 一致，但针对排序规则。
...
配置规则与 [`pg_locale`](#pg_locale) 一致，但针对字符类型。
...
默认值未定义，将使用 [`pg_cluster`](#pg_cluster) 的 SHA256 哈希值作为密钥。

你可以提供自定义的 [pgsodium](https://github.com/michelp/pgsodium) 密钥，应该是 64 位十六进制数字字符串。
...
默认值为 `pgsodium_getkey`，它将 [`roles/pgsql/templates/pgsodium_getkey`](https://github.com/pgsty/pigsty/blob/main/roles/pgsql/templates/pgsodium_getkey) 渲染到 `/pg/bin/pgsodium_getkey`。

默认的 getkey 脚本只是从 `/pg/conf/pgsodium.key` 读取 [`pgsodium_key`](#pgsodium_key) 并返回它。
如果你的密钥由外部系统（如 KMS、IAM 等）管理，你可以实现自己的 getkey 脚本从那里获取密钥：[示例](https://github.com/michelp/pgsodium/tree/main/getkey_scripts)。
...
- [默认角色](/zh/pgsql/acl/#默认角色)
- [默认用户](/zh/pgsql/acl/#默认用户)
- [默认权限](/zh/pgsql/acl/#默认权限)
- [默认HBA规则](/zh/pgsql/hba/#默认hba)
- 默认模式
- 默认扩展
...
Pigsty有一个内置的角色系统，请查看[PGSQL访问控制：角色系统](/zh/pgsql/acl/#角色系统)了解详情。
...
Pigsty 基于默认角色系统提供了相应的默认权限设置，请查看[PGSQL访问控制：权限](/zh/pgsql/acl/#默认权限)了解详情。
...
默认值为常见场景提供了足够的安全级别，请查看[PGSQL身份验证](/zh/pgsql/hba/)了解详情。

本参数为 [HBA](/zh/pgsql/hba/#定义hba)规则对象组成的数组，在形式上与 [`pg_hba_rules`](#pg_hba_rules) 完全一致。 建议在全局配置统一的 [`pg_default_hba_rules`](#pg_default_hba_rules)，针对特定集群使用 [`pg_hba_rules`](#pg_hba_rules) 进行额外定制。两个参数中的规则都会依次应用，后者优先级更高。
...
Pgbouncer 默认的基于主机的认证规则，数组或 [hba](/zh/pgsql/hba/#定义hba) 规则对象。

默认值提供了一套对于常见场景足够的安全级别，查看 [PGSQL Authentication](/zh/pgsql/hba/) 了解详情。
...
查看 [PGSQL 备份 & PITR](/zh/pgsql/pitr/) 以获取详细信息。
...
pgBackRest 日志目录，默认为 `/pg/log/pgbackrest`，[`promtail`](/zh/node/param/#promtail) 日志代理会引用此参数收集日志。
...
默认值： `100`。您必须在实例变量中定义它，并[重载服务](/zh/pgsql/admin/#重载服务)以生效。
...
当定义一个[服务](/zh/pgsql/svc/#定义服务)时，如果 `svc.dest='default'`，此参数将用作默认值。
...
默认值是四个默认服务定义，如[PGSQL Service](/zh/pgsql/svc/#服务概述)所述
...
例如，在默认配置下，存活类指标默认最多缓存 `1s`，大部分普通指标会缓存 `10s`（应当与 [`prometheus_scrape_interval`](/zh/infra/param#prometheus_scrape_interval) 相同）。
...
如果 [`pgbackrest_enabled`](#pgbackrest_enabled) 为 `false`，则本参数因短路无效。
