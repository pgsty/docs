...
这里的 Oracle 兼容性是指在 PL/SQL、语法、内置函数、数据类型、系统视图、MERGE 和 GUC 参数级别的兼容性。它不是像 [**Babelfish**](/zh/pgsql/kernel/babelfish)、[**openHalo**](/zh/pgsql/kernel/openhalo/) 或 [**FerretDB**](/zh/ferret/) 那样的线协议兼容性，不能使用原始客户端驱动程序。用户仍需要使用 PostgreSQL 客户端工具访问 IvorySQL，但可以使用 Oracle 兼容的语法。
