...
同时安装所有扩展是可行的（除了在 [`olap`](https://pgext.cloud/list/cate/#olap) 类别中的两个冲突）但不推荐。只需在 `pg_extensions` 参数中明确指定您需要的扩展。
...
默认情况下，Pigsty 还在这里指定了 3 个重要扩展：[`pgvector`](https://pgext.cloud/e/vector)、[`pg_repack`](https://pgext.cloud/e/pg_repack) 和 [`wal2json`](https://pgext.cloud/e/wal2json)，分别用于向量搜索、膨胀管理和 CDC 变更提取。
...
- 有两个已知冲突：
  - [`citus`](https://pgext.cloud/e/citus/) 和 [`hydra`](https://pgext.cloud/e/columnar) 是互斥的，因为 hydra 是 citus columnar 的分支，未重命名
  - 只从 [`pg_duckdb`](https://pgext.cloud/e/pg_duckdb)、[`pg_mooncake`](https://pgext.cloud/e/pg_mooncake)、[`duckdb_fdw`](https://pgext.cloud/e/duckdb_fdw) 中安装一个，它们都使用自己的 libduckdb
