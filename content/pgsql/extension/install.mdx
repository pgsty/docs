...
When installing extensions, Pigsty uses the same [**alias mapping**](/pgsql/extension/pkg) in the [**download**](/pgsql/extension/download) section.
...
Install all extensions simultaneously is applicable (except two conflicts in the [`olap`](https://pgext.cloud/list/cate/#olap) category) but not recommended. Just install the extensions you need by explicitly specifying them in the `pg_extensions` parameter.

------

## Configure

During PGSQL cluster init, Pigsty will automatically install packages (& alias) specified in [**`pg_packages`**](/pgsql/param#pg_packages) and [**`pg_extensions`**](/pgsql/param#pg_extensions).

- [**`pg_packages`**](/pgsql/param#pg_packages)
- [**`pg_extensions`**](/pgsql/param#pg_extensions)
...
By default, Pigsty also specifies 3 important extensions here: [`pgvector`](https://ext.pgsty.com/e/vector), [`pg_repack`](https://ext.pgsty.com/e/pg_repack), and [`wal2json`](https://ext.pgsty.com/e/wal2json) for vector search, bloat management, and CDC change extraction.
...
First, add extensions to [**`pg_extensions`**](/pgsql/param#pg_extensions), then execute the playbook subtask:
...
- **Local Repo**: You have configured using Pigsty’s local repo, and the extensions have already been [**downloaded**](/pgsql/extension/download/) to the local repo.
...
For production environments, we recommend using Pigsty’s local software repo to manage and install extensions uniformly: First [**download**](/pgsql/extension/download) extensions to the local repo, then install them from there.
...
When installing extensions, users can use [**extension aliases**](/pgsql/extension/pkg) to specify extension.

The aliases will be [**translated**](/pgsql/extension/pkg) to the current active PG major version and OS environment.

and translated to the corresponding RPM/DEB package names by [**alias translation mechanism**](/pgsql/extension/pkg).
...
- There are two known conflicts:
  - [`citus`](https://ext.pgsty.com/e/citus/) and [`hydra`](https://ext.pgsty.com/e/columnar) are mutually exclusive, since hydra is a fork of citus columnar without renaming
  - Only install one from [`pg_duckdb`](https://ext.pgsty.com/e/pg_duckdb), [`pg_mooncake`](https://ext.pgsty.com/e/pg_mooncake), [`duckdb_fdw`](https://ext.pgsty.com/e/duckdb_fdw), they all use their own libduckdb
