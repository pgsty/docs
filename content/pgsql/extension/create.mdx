...
Some built-in extensions and one special [`pg_repack`](https://pgext.cloud/e/pg_repack) are created by default in Pigsty.
...
|:----------------------------------------------:|-------------------------------|---------------------------------------------------------------------------------------------------------------------------|
| [`pg_repack`](https://pgext.cloud/e/pg_repack) | Online Bloat Control  Tools   | in the [`pg_default_extensions`](/pgsql/param#pg_default_extensions)                                                      |
|  [`wal2json`](https://pgext.cloud/e/wal2json)  | Changing data capture in JSON | extension [without DDL](https://ext.pgsty.com/list/attr#without-ddl), [install](/pgsql/extension/install) means available |
|    [`vector`](https://pgext.cloud/e/vector)    | vector data type & indexes    | in [`pg_databases`](/pgsql/param#pg_databases)  as an example                                                             |

The [`pg_repack`](https://pgext.cloud/e/pg_repack) extension is an important utility for maintaining bloat tables online.

[**`vector`**](https://pgext.cloud/e/vector) is a very popular extension for RAG,
...
The [`wal2json`](https://pgext.cloud/e/wal2json) is another important extension for Changing Data Capture (CDC). It is [installed](/pgsql/extension/install) by default, but it is an [**extension without DDL**](#extension-without-ddl),
...
[Extension without DDL](https://pgext.cloud/list/attr#without-ddl) does not require the `CREATE EXTENSION` command to work
