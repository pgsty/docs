...
All extensions listed in [`pg_extensions`](/pgsql/param#pg_extensions) will be upgraded using during the [`pgsql.yml`](/pgsql/playbook#pgsqlyml) playbook execution.
...
Extensions ([Package Alias](/pgsql/extension/pkg)) listed in [`pg_extensions`](/pgsql/param#pg_extensions) will be upgraded with [`pgsql.yml`](/pgsql/admin/)'s `pg_ext` subtask:
...
Pigsty’s [`pig`](https://ext.pgsty.com/pig/) cli can also help you with that, without the burden of specifying full package names:
