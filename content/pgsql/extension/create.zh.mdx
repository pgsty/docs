...
Pigsty 默认创建一些内置扩展和一个特殊的 [`pg_repack`](https://ext.pgsty.com/e/pg_repack)。
...
|                    扩展                      | 作用                          | 位置                                                                                                                      |
|:------------------------------------------------:|-------------------------------|---------------------------------------------------------------------------------------------------------------------------|
| [`pg_repack`](https://ext.pgsty.com/e/pg_repack) | 在线膨胀控制工具               | 在 [`pg_default_extensions`](/zh/pgsql/param#pg_default_extensions) 中                                                    |
|  [`wal2json`](https://ext.pgsty.com/e/wal2json)  | JSON 格式的变更数据捕获        | [无 DDL](https://ext.pgsty.com/list/attr#without-ddl) 扩展，[安装](/zh/pgsql/extension/install)意味着可用                |
|    [`vector`](https://ext.pgsty.com/e/vector)    | 向量数据类型和索引             | 在 [`pg_databases`](/zh/pgsql/param#pg_databases) 中作为示例                                                              |

[`pg_repack`](https://ext.pgsty.com/e/pg_repack) 扩展是在线维护膨胀表的重要工具。

[**`vector`**](https://ext.pgsty.com/e/vector) 是用于 RAG 的非常流行的扩展，
...
[`wal2json`](https://ext.pgsty.com/e/wal2json) 是用于变更数据捕获（CDC）的另一个重要扩展。它默认[安装](/zh/pgsql/extension/install)，但它是一个[**无 DDL 的扩展**](#extension-without-ddl)，
因此您无需显式 `CREATE` 它。
