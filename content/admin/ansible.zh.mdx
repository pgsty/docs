...
了解 Ansible 知识是好的，但**不是必需的**。您只需要知道如何运行 [**Ansible Playbooks**](/zh/admin/playbook)。
Playbooks 是包含要执行的一系列任务的可执行 YAML 文件。

运行 `./node.yml` playbook 本质上转换为 `ansible-playbook node.yml`。
...
| 目的                            | 参数                 | 描述                                           |
|------------------------------------|---------------------------|-------------------------------------------------------|
| [**在哪里**](#limit-host)           | `-l\|--limit <pattern>`   | 限制在特定组/主机/模式上的执行目标 |
| [**什么**](#limit-task)            | `-t\|--tags <tags>`       | 只运行具有特定标签的任务                     |
| [**如何**](#extra-vars)             | `-e\|--extra-vars <vars>` | 额外的命令行参数                          |
