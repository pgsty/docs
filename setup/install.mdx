...
{{% alert title="DO NOT use the root user" color="danger" %}}
While it is possible to install Pigsty as the `root` user, It would be much safer using a dedicate admin user (`dba`, `admin`, ...). due to [**security**](security) consideration
...
{{% tab header="Install Ansible" disabled=true /%}}
{{< tab header="EL 8 / 9" lang="Bash" >}}
sudo dnf install -y ansible python3.12-jmespath python3-cryptography
{{% /tab %}}
{{< tab header="EL 7" lang="Bash" >}}
...
{{% alert title="Checkout a specific version" color="danger" %}}
...
{{% alert title="Change the default passwords!" color="danger" %}}
PLEASE CHANGE THE DEFAULT PASSWORDs in the config file before installation, check [secure password](/docs/setup/security/#passwords) for details.{{% /alert %}}
...
{{% alert title="DON'T EVER RUN THIS PLAYBOOK AGAIN!" color="danger" %}}
It's very DANGEROUS to re-run [**`install.yml`**](https://github.com/Vonng/pigsty/blob/main/install.yml) on existing deployment!
